...
G_DEFINE_TYPE (SoliEncodingsDialog, soli_encodings_dialog, GTK_TYPE_DIALOG)

static void
set_modified (SoliEncodingsDialog *dialog)
{
...
static void
init_liststores (SoliEncodingsDialog *dialog,
		 gboolean              reset)
{
	...
	{
		chosen_encodings = soli_settings_get_candidate_encodings (&default_candidates);
	}
...
static GSList *
get_chosen_encodings_list (SoliEncodingsDialog *dialog)
{
...
static void
apply_settings (SoliEncodingsDialog *dialog)
{
	switch (dialog->state)
	{
		case STATE_MODIFIED:
		{
			...
			g_settings_set_strv (dialog->enc_settings,
					     SOLI_SETTINGS_CANDIDATE_ENCODINGS,
...
static void
soli_encodings_dialog_response (GtkDialog *gtk_dialog,
                                 gint       response_id)
{
	SoliEncodingsDialog *dialog = SOLI_ENCODINGS_DIALOG (gtk_dialog);
...
static void
soli_encodings_dialog_dispose (GObject *object)
{
	SoliEncodingsDialog *dialog = SOLI_ENCODINGS_DIALOG (object);
	...
	G_OBJECT_CLASS (soli_encodings_dialog_parent_class)->dispose (object);
}

static void
soli_encodings_dialog_class_init (SoliEncodingsDialogClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);
	GtkDialogClass *dialog_class = GTK_DIALOG_CLASS (klass);

	object_class->dispose = soli_encodings_dialog_dispose;

	dialog_class->response = soli_encodings_dialog_response;

	/* Bind class to template */
	gtk_widget_class_set_template_from_resource (widget_class,
	                                             "/ca/dluco/soli/ui/soli-encodings-dialog.ui");
	gtk_widget_class_bind_template_child (widget_class, SoliEncodingsDialog, liststore_available);
	gtk_widget_class_bind_template_child (widget_class, SoliEncodingsDialog, liststore_chosen);
	gtk_widget_class_bind_template_child (widget_class, SoliEncodingsDialog, sort_available);
	gtk_widget_class_bind_template_child (widget_class, SoliEncodingsDialog, treeview_available);
	gtk_widget_class_bind_template_child (widget_class, SoliEncodingsDialog, treeview_chosen);
...
static void
update_add_button_sensitivity (SoliEncodingsDialog *dialog)
{
...
static void
update_remove_button_sensitivity (SoliEncodingsDialog *dialog)
{
...
static void
update_up_down_buttons_sensitivity (SoliEncodingsDialog *dialog)
{
	GtkTreeSelection *selection;
	gint count;
	GList *selected_rows;
...
static void
update_chosen_buttons_sensitivity (SoliEncodingsDialog *dialog)
{
...
static void
init_toolbar_available (SoliEncodingsDialog *dialog)
{
...
static void
init_toolbar_chosen (SoliEncodingsDialog *dialog)
{
	GtkWidget *scrolled_window;
	GtkToolbar *toolbar;
	GtkStyleContext *context;
...
static void
soli_encodings_dialog_init (SoliEncodingsDialog *dialog)
{
	GtkTreeSelection *selection;

	dialog->enc_settings = g_settings_new ("ca.dluco.soli.preferences.encodings");
...
GtkWidget *
soli_encodings_dialog_new (void)
{
	return g_object_new (SOLI_TYPE_ENCODINGS_DIALOG,
			     "use-header-bar", TRUE,
			     NULL);
}
