...
struct _SoliMultiNotebookPrivate
{
	...
	SoliNotebookShowTabsModeType show_tabs_mode;
...
G_DEFINE_TYPE_WITH_PRIVATE (SoliMultiNotebook, soli_multi_notebook, GTK_TYPE_GRID)

static void	remove_notebook		(SoliMultiNotebook *mnb,
					 GtkWidget          *notebook);

static void	update_tabs_visibility	(SoliMultiNotebook *mnb);

static void
soli_multi_notebook_get_property (GObject    *object,
				   guint       prop_id,
				   GValue     *value,
				   GParamSpec *pspec)
{
	SoliMultiNotebook *mnb = SOLI_MULTI_NOTEBOOK (object);
...
static void
soli_multi_notebook_set_property (GObject      *object,
				   guint         prop_id,
				   const GValue *value,
				   GParamSpec   *pspec)
{
...
static void
soli_multi_notebook_dispose (GObject *object)
{
	SoliMultiNotebook *mnb = SOLI_MULTI_NOTEBOOK (object);

	g_clear_object (&mnb->priv->ui_settings);

	G_OBJECT_CLASS (soli_multi_notebook_parent_class)->dispose (object);
}

static void
soli_multi_notebook_finalize (GObject *object)
{
	SoliMultiNotebook *mnb = SOLI_MULTI_NOTEBOOK (object);

	g_list_free (mnb->priv->notebooks);

	G_OBJECT_CLASS (soli_multi_notebook_parent_class)->finalize (object);
}

static void
soli_multi_notebook_class_init (SoliMultiNotebookClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->dispose = soli_multi_notebook_dispose;
	object_class->finalize = soli_multi_notebook_finalize;
	object_class->get_property = soli_multi_notebook_get_property;
	object_class->set_property = soli_multi_notebook_set_property;
	...
	properties[PROP_SHOW_TABS_MODE] =
		g_param_spec_enum ("show-tabs-mode",
		                   ...
		                   SOLI_TYPE_NOTEBOOK_SHOW_TABS_MODE_TYPE,
	...
	signals[NOTEBOOK_ADDED] =
		g_signal_new ("notebook-added",
			      ...
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (SoliMultiNotebookClass, notebook_added),
			      NULL, NULL, NULL,
			      G_TYPE_NONE,
			      1,
			      SOLI_TYPE_NOTEBOOK);
	signals[NOTEBOOK_REMOVED] =
		g_signal_new ("notebook-removed",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (SoliMultiNotebookClass, notebook_removed),
			      NULL, NULL, NULL,
			      G_TYPE_NONE,
			      1,
			      SOLI_TYPE_NOTEBOOK);
	signals[TAB_ADDED] =
		g_signal_new ("tab-added",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (SoliMultiNotebookClass, tab_added),
			      NULL, NULL, NULL,
			      G_TYPE_NONE,
			      2,
			      SOLI_TYPE_NOTEBOOK,
	...
	signals[TAB_REMOVED] =
		g_signal_new ("tab-removed",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (SoliMultiNotebookClass, tab_removed),
			      NULL, NULL, NULL,
			      G_TYPE_NONE,
			      2,
			      SOLI_TYPE_NOTEBOOK,
	...
	signals[SWITCH_TAB] =
		g_signal_new ("switch-tab",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (SoliMultiNotebookClass, switch_tab),
			      NULL, NULL, NULL,
			      G_TYPE_NONE,
			      4,
			      SOLI_TYPE_NOTEBOOK,
	...
	signals[TAB_CLOSE_REQUEST] =
		g_signal_new ("tab-close-request",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (SoliMultiNotebookClass, tab_close_request),
			      NULL, NULL, NULL,
			      G_TYPE_NONE,
			      2,
			      SOLI_TYPE_NOTEBOOK,
	...
	signals[CREATE_WINDOW] =
		g_signal_new ("create-window",
		              ...
		              G_SIGNAL_RUN_LAST,
		              G_STRUCT_OFFSET (SoliMultiNotebookClass, create_window),
		              NULL, NULL, NULL,
		              GTK_TYPE_NOTEBOOK, 4,
		              SOLI_TYPE_NOTEBOOK, GTK_TYPE_WIDGET,
		              G_TYPE_INT, G_TYPE_INT);
	signals[PAGE_REORDERED] =
		g_signal_new ("page-reordered",
		              G_OBJECT_CLASS_TYPE (object_class),
		              G_SIGNAL_RUN_FIRST,
		              G_STRUCT_OFFSET (SoliMultiNotebookClass, page_reordered),
		              NULL, NULL, NULL,
		              G_TYPE_NONE,
		              3,
		              SOLI_TYPE_NOTEBOOK, GTK_TYPE_WIDGET,
	...
	signals[SHOW_POPUP_MENU] =
		g_signal_new ("show-popup-menu",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (SoliMultiNotebookClass, show_popup_menu),
			      NULL, NULL, NULL,
			      G_TYPE_NONE,
			      2,
...
static void
notebook_tab_close_request (SoliNotebook      *notebook,
			    SoliTab           *tab,
			    SoliMultiNotebook *mnb)
{
...
static GtkNotebook *
notebook_create_window (SoliNotebook      *notebook,
			GtkWidget          *child,
			gint                x,
			gint                y,
			SoliMultiNotebook *mnb)
...
static void
notebook_page_reordered (SoliNotebook      *notebook,
		         GtkWidget          *child,
		         guint               page_num,
		         SoliMultiNotebook *mnb)
{
...
static void
set_active_tab (SoliMultiNotebook *mnb,
                SoliTab           *tab)
{
		      ...
		      SoliMultiNotebook *mnb)
{
	...
	tab = SOLI_TAB (gtk_notebook_get_nth_page (book, page_num));
...
show_tabs_changed (GObject     *object,
		   ...
		   gpointer    *data)
{
	update_tabs_visibility (SOLI_MULTI_NOTEBOOK (data));
}

static void
update_tabs_visibility (SoliMultiNotebook *mnb)
{
	gboolean show_tabs;
	GList *l;
	...
	else if (mnb->priv->notebooks->next == NULL) /* only one notebook */
	{
		...
		{
			case SOLI_NOTEBOOK_SHOW_TABS_NEVER:
				show_tabs = FALSE;
				break;
			case SOLI_NOTEBOOK_SHOW_TABS_AUTO:
				show_tabs = gtk_notebook_get_n_pages (GTK_NOTEBOOK (mnb->priv->notebooks->data)) > 1;
				break;
			case SOLI_NOTEBOOK_SHOW_TABS_ALWAYS:
			default:
				show_tabs = TRUE;
				break;
		}
	...
	{
		show_tabs = (mnb->priv->show_tabs_mode != SOLI_NOTEBOOK_SHOW_TABS_NEVER);
	}
...
static void
connect_notebook_signals (SoliMultiNotebook *mnb,
			  GtkWidget          *notebook)
{
...
static void
disconnect_notebook_signals (SoliMultiNotebook *mnb,
			     GtkWidget          *notebook)
{
...
static void
add_notebook (SoliMultiNotebook *mnb,
	      GtkWidget          *notebook,
...
static void
remove_notebook (SoliMultiNotebook *mnb,
		 GtkWidget          *notebook)
{
	GtkWidget *parent;
	GtkWidget *grandpa;
...
static void
soli_multi_notebook_init (SoliMultiNotebook *mnb)
{
	SoliMultiNotebookPrivate *priv;

	mnb->priv = soli_multi_notebook_get_instance_private (mnb);
	priv = mnb->priv;
	...
	priv->show_tabs_mode = SOLI_NOTEBOOK_SHOW_TABS_ALWAYS;
	priv->show_tabs = TRUE;

	priv->ui_settings = g_settings_new ("ca.dluco.soli.preferences.editor");
	g_settings_bind (priv->ui_settings,
			 SOLI_SETTINGS_SHOW_TABS_MODE,
			 mnb,
			 "show-tabs-mode",
			 G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);

	priv->active_notebook = soli_notebook_new ();
	add_notebook (mnb, priv->active_notebook, TRUE);
}

SoliMultiNotebook *
soli_multi_notebook_new ()
{
	return g_object_new (SOLI_TYPE_MULTI_NOTEBOOK, NULL);
}

SoliNotebook *
soli_multi_notebook_get_active_notebook (SoliMultiNotebook *mnb)
{
	g_return_val_if_fail (SOLI_IS_MULTI_NOTEBOOK (mnb), NULL);

	return SOLI_NOTEBOOK (mnb->priv->active_notebook);
}

gint
soli_multi_notebook_get_n_notebooks (SoliMultiNotebook *mnb)
{
	g_return_val_if_fail (SOLI_IS_MULTI_NOTEBOOK (mnb), 0);

	return g_list_length (mnb->priv->notebooks);
}

SoliNotebook *
soli_multi_notebook_get_nth_notebook (SoliMultiNotebook *mnb,
				       gint                notebook_num)
{
	g_return_val_if_fail (SOLI_IS_MULTI_NOTEBOOK (mnb), NULL);
...
SoliNotebook *
soli_multi_notebook_get_notebook_for_tab (SoliMultiNotebook *mnb,
                                           SoliTab           *tab)
{
	GList *l;
	gint page_num;
	...
	return SOLI_NOTEBOOK (l->data);
}

gint
soli_multi_notebook_get_notebook_num (SoliMultiNotebook *mnb,
				       SoliNotebook      *notebook)
{
	g_return_val_if_fail (SOLI_IS_MULTI_NOTEBOOK (mnb), -1);
	g_return_val_if_fail (SOLI_IS_NOTEBOOK (notebook), -1);

	return g_list_index (mnb->priv->notebooks, notebook);
}

gint
soli_multi_notebook_get_n_tabs (SoliMultiNotebook *mnb)
{
	g_return_val_if_fail (SOLI_IS_MULTI_NOTEBOOK (mnb), 0);
...
gint
soli_multi_notebook_get_page_num (SoliMultiNotebook *mnb,
				   SoliTab           *tab)
{
	GList *l;
...
SoliTab *
soli_multi_notebook_get_active_tab (SoliMultiNotebook *mnb)
{
	g_return_val_if_fail (SOLI_IS_MULTI_NOTEBOOK (mnb), NULL);

	return (mnb->priv->active_tab == NULL) ?
				NULL : SOLI_TAB (mnb->priv->active_tab);
}

void
soli_multi_notebook_set_active_tab (SoliMultiNotebook *mnb,
				     SoliTab           *tab)
{
	GList *l;
	gint page_num;

	g_return_if_fail (SOLI_IS_MULTI_NOTEBOOK (mnb));
	g_return_if_fail (SOLI_IS_TAB (tab) || tab == NULL);

	/* use plain C cast since the active tab can be null */
	if (tab == (SoliTab *) mnb->priv->active_tab)
	{
		return;
	}
...
void
soli_multi_notebook_set_current_page (SoliMultiNotebook *mnb,
				       gint                page_num)
{
	GList *l;
	gint pages = 0;
...
GList *
soli_multi_notebook_get_all_tabs (SoliMultiNotebook *mnb)
{
	GList *nbs;
	GList *ret = NULL;
...
void
soli_multi_notebook_close_tabs (SoliMultiNotebook *mnb,
				 const GList        *tabs)
{
	GList *l;
...
/**
 * soli_multi_notebook_close_all_tabs:
 * @mnb: a #SoliMultiNotebook
 *
 * Closes all opened tabs.
 */
void
soli_multi_notebook_close_all_tabs (SoliMultiNotebook *mnb)
{
	GList *nbs, *l;

	g_return_if_fail (SOLI_MULTI_NOTEBOOK (mnb));
	...
	{
		soli_notebook_remove_all_tabs (SOLI_NOTEBOOK (l->data));
	}
...
void
soli_multi_notebook_add_new_notebook (SoliMultiNotebook *mnb)
{
	GtkWidget *notebook;
	SoliTab *tab;
...
void
soli_multi_notebook_add_new_notebook_with_tab (SoliMultiNotebook *mnb,
                                                SoliTab           *tab)
{
	GtkWidget *notebook;
	...
	old_notebook = soli_multi_notebook_get_notebook_for_tab (mnb, tab);
...
void
soli_multi_notebook_remove_active_notebook (SoliMultiNotebook *mnb)
{
	g_return_if_fail (SOLI_IS_MULTI_NOTEBOOK (mnb));

	soli_notebook_remove_all_tabs (SOLI_NOTEBOOK (mnb->priv->active_notebook));
}

void
soli_multi_notebook_previous_notebook (SoliMultiNotebook *mnb)
{
	GList *current;
	GtkWidget *notebook;
...
void
soli_multi_notebook_next_notebook (SoliMultiNotebook *mnb)
{
	GList *current;
	GtkWidget *notebook;
...
void
soli_multi_notebook_foreach_notebook (SoliMultiNotebook *mnb,
...
void
soli_multi_notebook_foreach_tab (SoliMultiNotebook *mnb,
				  GtkCallback         callback,
				  gpointer            callback_data)
{
	GList *nb;
 ...
 */
void
_soli_multi_notebook_set_show_tabs (SoliMultiNotebook *mnb,
				     gboolean           show)
{
	g_return_if_fail (SOLI_IS_MULTI_NOTEBOOK (mnb));
