...
struct _SoliNotebookStackSwitcherPrivate
{
	GtkWidget *notebook;
	GtkStack *stack;
};
...
G_DEFINE_TYPE_WITH_PRIVATE (SoliNotebookStackSwitcher, soli_notebook_stack_switcher, GTK_TYPE_BIN)

static void
soli_notebook_stack_switcher_init (SoliNotebookStackSwitcher *switcher)
{
	SoliNotebookStackSwitcherPrivate *priv;

	priv = soli_notebook_stack_switcher_get_instance_private (switcher);
	switcher->priv = priv;
...
static GtkWidget *
find_notebook_child (SoliNotebookStackSwitcher *switcher,
		     GtkWidget                  *stack_child)
{
	SoliNotebookStackSwitcherPrivate *priv = switcher->priv;
	GList *pages;
	GList *p;
	GtkWidget *ret = NULL;
...
static void
sync_label (SoliNotebookStackSwitcher *switcher,
	    GtkWidget                  *stack_child,
...
on_child_prop_changed (GtkWidget                  *widget,
                       GParamSpec                 *pspec,
                       SoliNotebookStackSwitcher *switcher)
{
	GtkWidget *nb_child;
                         ...
                         SoliNotebookStackSwitcher *switcher)
{
	 ...
	 * to avoid an assertion when closing soli, since the remove signal
...
static void
disconnect_signals (SoliNotebookStackSwitcher *switcher)
{
	SoliNotebookStackSwitcherPrivate *priv = switcher->priv;
...
static void
connect_signals (SoliNotebookStackSwitcher *switcher)
{
	SoliNotebookStackSwitcherPrivate *priv = switcher->priv;
...
void
soli_notebook_stack_switcher_set_stack (SoliNotebookStackSwitcher *switcher,
                                         GtkStack                   *stack)
{
	SoliNotebookStackSwitcherPrivate *priv;

	g_return_if_fail (SOLI_IS_NOTEBOOK_STACK_SWITCHER (switcher));
...
GtkStack *
soli_notebook_stack_switcher_get_stack (SoliNotebookStackSwitcher *switcher)
{
	g_return_val_if_fail (SOLI_IS_NOTEBOOK_STACK_SWITCHER (switcher), NULL);

	return switcher->priv->stack;
}

static void
soli_notebook_stack_switcher_get_property (GObject    *object,
                                            guint       prop_id,
                                            GValue     *value,
                                            GParamSpec *pspec)
{
	SoliNotebookStackSwitcher *switcher = SOLI_NOTEBOOK_STACK_SWITCHER (object);
	SoliNotebookStackSwitcherPrivate *priv = switcher->priv;
...
static void
soli_notebook_stack_switcher_set_property (GObject      *object,
                                            guint         prop_id,
                                            const GValue *value,
                                            GParamSpec   *pspec)
{
	...
	{
		case PROP_STACK:
			soli_notebook_stack_switcher_set_stack (switcher, g_value_get_object (value));
			break;
...
static void
soli_notebook_stack_switcher_dispose (GObject *object)
{
	SoliNotebookStackSwitcher *switcher = SOLI_NOTEBOOK_STACK_SWITCHER (object);

	soli_notebook_stack_switcher_set_stack (switcher, NULL);

	G_OBJECT_CLASS (soli_notebook_stack_switcher_parent_class)->dispose (object);
}

static void
soli_notebook_stack_switcher_class_init (SoliNotebookStackSwitcherClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->get_property = soli_notebook_stack_switcher_get_property;
	object_class->set_property = soli_notebook_stack_switcher_set_property;
	object_class->dispose = soli_notebook_stack_switcher_dispose;
...
GtkWidget *
soli_notebook_stack_switcher_new (void)
{
	return g_object_new (SOLI_TYPE_NOTEBOOK_STACK_SWITCHER, NULL);
}
