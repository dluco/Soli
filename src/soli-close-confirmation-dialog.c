...
#define SOLI_SAVE_DOCUMENT_KEY "soli-save-document"

struct _SoliCloseConfirmationDialog
{
...
G_DEFINE_TYPE (SoliCloseConfirmationDialog,
	       soli_close_confirmation_dialog,
	       GTK_TYPE_MESSAGE_DIALOG)

static void 	 set_unsaved_document 		(SoliCloseConfirmationDialog *dlg,
						 const GList                  *list);
...
get_selected_docs (GtkWidget *list_box)
{
	...
	for (l = rows; l != NULL; l = l->next)
	{
		...
		if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (check_button)))
		{
			...
			doc = g_object_get_data (G_OBJECT (row), SOLI_SAVE_DOCUMENT_KEY);
 ...
 */
static void
response_cb (SoliCloseConfirmationDialog *dlg,
             gint                          response_id,
             gpointer                      data)
{
	g_return_if_fail (SOLI_IS_CLOSE_CONFIRMATION_DIALOG (dlg));
...
static void
soli_close_confirmation_dialog_init (SoliCloseConfirmationDialog *dlg)
{
	SoliLockdownMask lockdown;

	lockdown = soli_app_get_lockdown (SOLI_APP (g_application_get_default ()));

	dlg->disable_save_to_disk = (lockdown & SOLI_LOCKDOWN_SAVE_TO_DISK) != 0;
...
static void
soli_close_confirmation_dialog_finalize (GObject *object)
{
	SoliCloseConfirmationDialog *dlg = SOLI_CLOSE_CONFIRMATION_DIALOG (object);

	g_list_free (dlg->unsaved_documents);
	g_list_free (dlg->selected_documents);

	/* Call the parent's destructor */
	G_OBJECT_CLASS (soli_close_confirmation_dialog_parent_class)->finalize (object);
}

static void
soli_close_confirmation_dialog_set_property (GObject      *object,
					      guint         prop_id,
					      const GValue *value,
					      GParamSpec   *pspec)
{
	SoliCloseConfirmationDialog *dlg;

	dlg = SOLI_CLOSE_CONFIRMATION_DIALOG (object);
...
static void
soli_close_confirmation_dialog_get_property (GObject    *object,
					      guint       prop_id,
					      GValue     *value,
					      GParamSpec *pspec)
{
...
static void
soli_close_confirmation_dialog_class_init (SoliCloseConfirmationDialogClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	gobject_class->set_property = soli_close_confirmation_dialog_set_property;
	gobject_class->get_property = soli_close_confirmation_dialog_get_property;
	gobject_class->finalize = soli_close_confirmation_dialog_finalize;
...
GList *
soli_close_confirmation_dialog_get_selected_documents (SoliCloseConfirmationDialog *dlg)
{
	g_return_val_if_fail (SOLI_IS_CLOSE_CONFIRMATION_DIALOG (dlg), NULL);

	return g_list_copy (dlg->selected_documents);
}

GtkWidget *
soli_close_confirmation_dialog_new (GtkWindow *parent,
				     GList     *unsaved_documents)
{
	GtkWidget *dlg;
	...
	dlg = g_object_new (SOLI_TYPE_CLOSE_CONFIRMATION_DIALOG,
	...
	if (parent != NULL)
	{
		gtk_window_group_add_window (soli_window_get_group (SOLI_WINDOW (parent)),
					     GTK_WINDOW (dlg));
...
GtkWidget *
soli_close_confirmation_dialog_new_single (GtkWindow     *parent,
					    SoliDocument *doc)
{
	GtkWidget *dlg;
	GList *unsaved_documents;
	...
	dlg = soli_close_confirmation_dialog_new (parent, unsaved_documents);
...
static void
add_buttons (SoliCloseConfirmationDialog *dlg)
{
	...
	else
	{
		...
		if (GET_MODE (dlg) == SINGLE_DOC_MODE)
		{
			...
			doc = SOLI_DOCUMENT (dlg->unsaved_documents->data);
...
static gchar *
get_text_secondary_label (SoliDocument *doc)
{
	glong  seconds;
	gchar *secondary_msg;
...
static void
build_single_doc_dialog (SoliCloseConfirmationDialog *dlg)
{
	SoliDocument *doc;
	gchar *doc_name;
	gchar *str;
	...
	/* Primary message */
	doc_name = soli_document_get_short_name_for_display (doc);
...
static GtkWidget *
create_list_box (SoliCloseConfirmationDialog *dlg)
{
	GtkWidget *list_box;
	GList *l;
...
static void
build_multiple_docs_dialog (SoliCloseConfirmationDialog *dlg)
{
...
static void
set_unsaved_document (SoliCloseConfirmationDialog *dlg,
		      const GList                  *list)
{
...
const GList *
soli_close_confirmation_dialog_get_unsaved_documents (SoliCloseConfirmationDialog *dlg)
{
	g_return_val_if_fail (SOLI_IS_CLOSE_CONFIRMATION_DIALOG (dlg), NULL);
