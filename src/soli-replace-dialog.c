...
#define SOLI_SEARCH_CONTEXT_KEY "soli-search-context-key"

struct _SoliReplaceDialog
{
	...
	SoliDocument *active_document;

	guint idle_update_sensitivity_id;
};

G_DEFINE_TYPE (SoliReplaceDialog, soli_replace_dialog, GTK_TYPE_DIALOG)

static GtkSourceSearchContext *
get_search_context (SoliReplaceDialog *dialog,
		    SoliDocument      *doc)
{
	...
	if (search_context != NULL &&
	    g_object_get_data (G_OBJECT (search_context), SOLI_SEARCH_CONTEXT_KEY) == dialog)
	{
 ...
 * execute the search.
 *
 * Likewise, each created SearchContext (from the SoliReplaceDialog) contains a
 ...
 */
static void
set_search_settings (SoliReplaceDialog *dialog)
{
	GtkSourceSearchContext *search_context;
...
static SoliWindow *
get_soli_window (SoliReplaceDialog *dialog)
{
	GtkWindow *transient_for = gtk_window_get_transient_for (GTK_WINDOW (dialog));

	return transient_for != NULL ? SOLI_WINDOW (transient_for) : NULL;
}

static SoliDocument *
get_active_document (SoliReplaceDialog *dialog)
{
	SoliWindow *window = get_soli_window (dialog);

	return window != NULL ? soli_window_get_active_document (window) : NULL;
}

void
soli_replace_dialog_present_with_time (SoliReplaceDialog *dialog,
					guint32             timestamp)
{
	g_return_if_fail (SOLI_REPLACE_DIALOG (dialog));
...
static gboolean
soli_replace_dialog_delete_event (GtkWidget   *widget,
                                   GdkEventAny *event)
{
	/* prevent destruction */
	return TRUE;
...
static void
set_search_error (SoliReplaceDialog *dialog,
		  const gchar        *error_msg)
{
...
void
soli_replace_dialog_set_replace_error (SoliReplaceDialog *dialog,
					const gchar        *error_msg)
{
...
static gboolean
has_search_error (SoliReplaceDialog *dialog)
{
	GIcon *icon;
...
static gboolean
has_replace_error (SoliReplaceDialog *dialog)
{
	GIcon *icon;
...
static void
update_regex_error (SoliReplaceDialog *dialog)
{
	GtkSourceSearchContext *search_context;
	GError *regex_error;
...
static gboolean
update_replace_response_sensitivity_cb (SoliReplaceDialog *dialog)
{
	GtkSourceSearchContext *search_context;
	GtkTextIter start;
	GtkTextIter end;
...
static void
install_idle_update_sensitivity (SoliReplaceDialog *dialog)
{
...
static void
update_responses_sensitivity (SoliReplaceDialog *dialog)
{
	const gchar *search_text;
	...
	{
		gtk_dialog_set_response_sensitive (GTK_DIALOG (dialog),
						   SOLI_REPLACE_DIALOG_FIND_RESPONSE,
						   FALSE);
...
static void
regex_error_notify_cb (SoliReplaceDialog *dialog)
{
...
static void
disconnect_document (SoliReplaceDialog *dialog)
{
	GtkSourceSearchContext *search_context;
...
static void
connect_active_document (SoliReplaceDialog *dialog)
{
	SoliDocument *doc;
	GtkSourceSearchContext *search_context;
	...
	if (search_context == NULL)
	{
		...
		g_object_set_data (G_OBJECT (search_context),
				   SOLI_SEARCH_CONTEXT_KEY,
				   dialog);

		soli_document_set_search_context (doc, search_context);
...
response_cb (GtkDialog *dialog,
	     gint       response_id)
{
	SoliReplaceDialog *dlg = SOLI_REPLACE_DIALOG (dialog);
	const gchar *str;
	...
	switch (response_id)
	{
		...
		case SOLI_REPLACE_DIALOG_REPLACE_ALL_RESPONSE:
			...
			if (*str != '\0')
			{
				soli_history_entry_prepend_text
						(SOLI_HISTORY_ENTRY (dlg->replace_entry),
						 str);
			}
		...
		case SOLI_REPLACE_DIALOG_FIND_RESPONSE:
			...
			{
				soli_history_entry_prepend_text
						(SOLI_HISTORY_ENTRY (dlg->search_entry),
						 str);
			}
	}
	...
	switch (response_id)
	{
		...
		case SOLI_REPLACE_DIALOG_REPLACE_ALL_RESPONSE:
		case SOLI_REPLACE_DIALOG_FIND_RESPONSE:
			connect_active_document (SOLI_REPLACE_DIALOG (dialog));
			set_search_settings (SOLI_REPLACE_DIALOG (dialog));
	}
}

static void
soli_replace_dialog_dispose (GObject *object)
{
	SoliReplaceDialog *dialog = SOLI_REPLACE_DIALOG (object);
	...
	G_OBJECT_CLASS (soli_replace_dialog_parent_class)->dispose (object);
}

static void
soli_replace_dialog_class_init (SoliReplaceDialogClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
	GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

	gobject_class->dispose = soli_replace_dialog_dispose;
	widget_class->delete_event = soli_replace_dialog_delete_event;

	/* Bind class to template */
	gtk_widget_class_set_template_from_resource (widget_class,
	                                             "/ca/dluco/soli/ui/soli-replace-dialog.ui");
	gtk_widget_class_bind_template_child (widget_class, SoliReplaceDialog, grid);
	gtk_widget_class_bind_template_child (widget_class, SoliReplaceDialog, search_label);
	gtk_widget_class_bind_template_child (widget_class, SoliReplaceDialog, replace_label);
	gtk_widget_class_bind_template_child (widget_class, SoliReplaceDialog, match_case_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliReplaceDialog, entire_word_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliReplaceDialog, regex_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliReplaceDialog, backwards_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliReplaceDialog, wrap_around_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliReplaceDialog, close_button);
}
...
replace_text_entry_changed (GtkEditable        *editable,
			    SoliReplaceDialog *dialog)
{
	soli_replace_dialog_set_replace_error (dialog, NULL);
...
/* TODO: move in soli-document.c and share it with soli-view-frame */
static gboolean
...
static void
show_cb (SoliReplaceDialog *dialog)
{
	SoliWindow *window;
	SoliDocument *doc;
...
static void
hide_cb (SoliReplaceDialog *dialog)
{
	disconnect_document (dialog);
}

static void
soli_replace_dialog_init (SoliReplaceDialog *dlg)
{
	gtk_widget_init_template (GTK_WIDGET (dlg));

	dlg->search_entry = soli_history_entry_new ("search-for-entry", TRUE);
	gtk_widget_set_size_request (dlg->search_entry, 300, -1);
	gtk_widget_set_hexpand (GTK_WIDGET (dlg->search_entry), TRUE);
	dlg->search_text_entry = soli_history_entry_get_entry (SOLI_HISTORY_ENTRY (dlg->search_entry));
	...
	dlg->replace_entry = soli_history_entry_new ("replace-with-entry", TRUE);
	gtk_widget_set_hexpand (GTK_WIDGET (dlg->replace_entry), TRUE);
	dlg->replace_text_entry = soli_history_entry_get_entry (SOLI_HISTORY_ENTRY (dlg->replace_entry));
...
GtkWidget *
soli_replace_dialog_new (SoliWindow *window)
{
	SoliReplaceDialog *dialog;
	gboolean use_header;
	...
	dialog = g_object_new (SOLI_TYPE_REPLACE_DIALOG,
...
const gchar *
soli_replace_dialog_get_replace_text (SoliReplaceDialog *dialog)
{
	g_return_val_if_fail (SOLI_IS_REPLACE_DIALOG (dialog), NULL);

	return gtk_entry_get_text (GTK_ENTRY (dialog->replace_text_entry));
}

gboolean
soli_replace_dialog_get_backwards (SoliReplaceDialog *dialog)
{
	g_return_val_if_fail (SOLI_IS_REPLACE_DIALOG (dialog), FALSE);
 ...
 */
const gchar *
soli_replace_dialog_get_search_text (SoliReplaceDialog *dialog)
{
	g_return_val_if_fail (SOLI_IS_REPLACE_DIALOG (dialog), NULL);
