...
struct _SoliHighlightModeSelector
{
...
G_DEFINE_TYPE (SoliHighlightModeSelector, soli_highlight_mode_selector, GTK_TYPE_GRID)

static void
soli_highlight_mode_selector_language_selected (SoliHighlightModeSelector *widget,
                                                 GtkSourceLanguage          *language)
{
}

static void
soli_highlight_mode_selector_class_init (SoliHighlightModeSelectorClass *klass)
{
	...
	signals[LANGUAGE_SELECTED] =
		g_signal_new_class_handler ("language-selected",
		                            ...
		                            G_CALLBACK (soli_highlight_mode_selector_language_selected),
		                            NULL, NULL, NULL,
		                            G_TYPE_NONE,
		                            1,
	...
	gtk_widget_class_set_template_from_resource (widget_class,
	                                             "/ca/dluco/soli/ui/soli-highlight-mode-selector.ui");
	gtk_widget_class_bind_template_child (widget_class, SoliHighlightModeSelector, treeview);
	gtk_widget_class_bind_template_child (widget_class, SoliHighlightModeSelector, entry);
	gtk_widget_class_bind_template_child (widget_class, SoliHighlightModeSelector, liststore);
	gtk_widget_class_bind_template_child (widget_class, SoliHighlightModeSelector, treemodelfilter);
	gtk_widget_class_bind_template_child (widget_class, SoliHighlightModeSelector, treeview_selection);
}
...
on_entry_activate (GtkEntry                   *entry,
                   SoliHighlightModeSelector *selector)
{
	soli_highlight_mode_selector_activate_selected_language (selector);
}
...
static gboolean
move_selection (SoliHighlightModeSelector *selector,
                gint                      howmany)
{
	GtkTreeIter iter;
	GtkTreePath *path;
...
static void
soli_highlight_mode_selector_init (SoliHighlightModeSelector *selector)
{
	...
	selector = soli_highlight_mode_selector_get_instance_private (selector);
...
soli_highlight_mode_selector_new ()
{
	return g_object_new (SOLI_TYPE_HIGHLIGHT_MODE_SELECTOR, NULL);
}

void
soli_highlight_mode_selector_select_language (SoliHighlightModeSelector *selector,
                                             GtkSourceLanguage        *language)
{
	GtkTreeIter iter;

	g_return_if_fail (SOLI_IS_HIGHLIGHT_MODE_SELECTOR (selector));
...
void
soli_highlight_mode_selector_activate_selected_language (SoliHighlightModeSelector *selector)
{
	GtkSourceLanguage *lang;
	GtkTreeIter iter;
