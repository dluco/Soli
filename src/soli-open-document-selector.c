...
#include "soli-open-document-selector-store.h"
...
struct _SoliOpenDocumentSelector
{
	...
	SoliOpenDocumentSelectorStore *selector_store;
	GList *recent_items;
...
G_DEFINE_TYPE (SoliOpenDocumentSelector, soli_open_document_selector, GTK_TYPE_BOX)
...
static void
create_row (SoliOpenDocumentSelector *selector,
            const FileItem            *item,
            GRegex                    *filter_regex)
{
	GtkTreeIter iter;
...
static GList *
compute_all_items_list (SoliOpenDocumentSelector *selector)
{
	GList *recent_items;
	GList *home_dir_items;
	GList *desktop_dir_items;
	...
	recent_items = soli_open_document_selector_copy_file_items_list ((const GList *)selector->recent_items);
	home_dir_items = soli_open_document_selector_copy_file_items_list ((const GList *)selector->home_dir_items);
	desktop_dir_items = soli_open_document_selector_copy_file_items_list ((const GList *)selector->desktop_dir_items);
	local_bookmarks_dir_items = soli_open_document_selector_copy_file_items_list ((const GList *)selector->local_bookmarks_dir_items);
	file_browser_root_items = soli_open_document_selector_copy_file_items_list ((const GList *)selector->file_browser_root_items);
	active_doc_dir_items = soli_open_document_selector_copy_file_items_list ((const GList *)selector->active_doc_dir_items);
	current_docs_items = soli_open_document_selector_copy_file_items_list ((const GList *)selector->current_docs_items);

	if (selector->all_items)
	{
		soli_open_document_selector_free_file_items_list (selector->all_items);
                         ...
                         gint   limit)
{
	...
	{
		item = soli_open_document_selector_copy_fileitem_item (l->data);
...
fileitem_setup (FileItem *item)
{
	...
	else
	{
		...
		item->path = soli_utils_location_get_dirname_for_display (file);
		item->name  = soli_utils_basename_for_display (file);
	...
	for (l = items; l != NULL; l = l->next)
	{
		...
		if (candidate && (filter ==  NULL || strstr (candidate, filter)))
		{
			new_items = g_list_prepend (new_items,
			                            soli_open_document_selector_copy_fileitem_item (item));
		}
...
fileitem_list_remove_duplicates (GList *items)
{
	...
	while (l != NULL)
	{
		...
		{
			soli_open_document_selector_free_fileitem_item ((FileItem *)l1->data);
...
{
	SoliOpenDocumentSelector *selector = SOLI_OPEN_DOCUMENT_SELECTOR (data);
	SoliOpenDocumentSelectorStore *selector_store;
	GList *l;
	...
	filter = soli_open_document_selector_store_get_filter (selector_store);
	if (filter && *filter != '\0')
	{
	...
	else
	{
		...
		recent_limit = soli_open_document_selector_store_get_recent_limit (selector_store);
		...
		if (recent_limit > 0 )
		{
			...
			soli_open_document_selector_free_file_items_list (recent_items);
		}
		else
		{
	...
	soli_open_document_selector_free_file_items_list (filter_items);
...
static void
populate_liststore (SoliOpenDocumentSelector *selector)
{
...
on_treeview_key_press (GtkTreeView               *treeview,
                       GdkEventKey               *event,
                       SoliOpenDocumentSelector *selector)
{
	guint keyval;
                  ...
                  SoliOpenDocumentSelector *selector)
{
	...
	soli_open_document_selector_store_set_filter (selector->selector_store,
	                                               entry_text);
...
static void
soli_open_document_selector_dispose (GObject *object)
{
	SoliOpenDocumentSelector *selector = SOLI_OPEN_DOCUMENT_SELECTOR (object);
	...
	{
		soli_open_document_selector_free_file_items_list (selector->recent_items);
		selector->recent_items = NULL;
	}

	if (selector->home_dir_items)
	{
		soli_open_document_selector_free_file_items_list (selector->home_dir_items);
		selector->home_dir_items = NULL;
	}

	if (selector->desktop_dir_items)
	{
		soli_open_document_selector_free_file_items_list (selector->desktop_dir_items);
		selector->desktop_dir_items = NULL;
	}

	if (selector->local_bookmarks_dir_items)
	{
		soli_open_document_selector_free_file_items_list (selector->local_bookmarks_dir_items);
		selector->local_bookmarks_dir_items = NULL;
	}

	if (selector->file_browser_root_items)
	{
		soli_open_document_selector_free_file_items_list (selector->file_browser_root_items);
		selector->file_browser_root_items = NULL;
	}

	if (selector->active_doc_dir_items)
	{
		soli_open_document_selector_free_file_items_list (selector->active_doc_dir_items);
		selector->active_doc_dir_items = NULL;
	}

	if (selector->current_docs_items)
	{
		soli_open_document_selector_free_file_items_list (selector->current_docs_items);
	...
	G_OBJECT_CLASS (soli_open_document_selector_parent_class)->dispose (object);
}
...
static void
update_list_cb (SoliOpenDocumentSelectorStore *selector_store,
                GAsyncResult                   *res,
                gpointer                        user_data G_GNUC_UNUSED)
{
	...
	list = soli_open_document_selector_store_update_list_finish (selector_store, res, &error);
	...
	switch (type)
	{
		case SOLI_OPEN_DOCUMENT_SELECTOR_RECENT_FILES_LIST:
			soli_open_document_selector_free_file_items_list (selector->recent_items);
			selector->recent_items = list;
			break;

		case SOLI_OPEN_DOCUMENT_SELECTOR_HOME_DIR_LIST:
			soli_open_document_selector_free_file_items_list (selector->home_dir_items);
			selector->home_dir_items = list;
			break;

		case SOLI_OPEN_DOCUMENT_SELECTOR_DESKTOP_DIR_LIST:
			soli_open_document_selector_free_file_items_list (selector->desktop_dir_items);
			selector->desktop_dir_items = list;
			break;

		case SOLI_OPEN_DOCUMENT_SELECTOR_LOCAL_BOOKMARKS_DIR_LIST:
			soli_open_document_selector_free_file_items_list (selector->local_bookmarks_dir_items);
			selector->local_bookmarks_dir_items = list;
			break;

		case SOLI_OPEN_DOCUMENT_SELECTOR_FILE_BROWSER_ROOT_DIR_LIST:
			soli_open_document_selector_free_file_items_list (selector->file_browser_root_items);
			selector->file_browser_root_items = list;
			break;

		case SOLI_OPEN_DOCUMENT_SELECTOR_ACTIVE_DOC_DIR_LIST:
			soli_open_document_selector_free_file_items_list (selector->active_doc_dir_items);
			selector->active_doc_dir_items = list;
			break;

		case SOLI_OPEN_DOCUMENT_SELECTOR_CURRENT_DOCS_LIST:
			soli_open_document_selector_free_file_items_list (selector->current_docs_items);
...
static void
soli_open_document_selector_constructed (GObject *object)
{
	SoliOpenDocumentSelector *selector = SOLI_OPEN_DOCUMENT_SELECTOR (object);

	G_OBJECT_CLASS (soli_open_document_selector_parent_class)->constructed (object);

	DEBUG_SELECTOR (g_print ("Selector(%p): constructed - ask recent file list\n", selector););

	soli_open_document_selector_store_update_list_async (selector->selector_store,
	                                                      selector,
	                                                      NULL,
	                                                      (GAsyncReadyCallback)update_list_cb,
	                                                      SOLI_OPEN_DOCUMENT_SELECTOR_RECENT_FILES_LIST,
	                                                      selector);
}

static void
soli_open_document_selector_mapped (GtkWidget *widget)
{
	SoliOpenDocumentSelector *selector = SOLI_OPEN_DOCUMENT_SELECTOR (widget);
	ListType list_number;

	/* We update all the lists */
	DEBUG_SELECTOR (g_print ("Selector(%p): mapped - ask all lists\n", selector););

	for (list_number = 0; list_number < SOLI_OPEN_DOCUMENT_SELECTOR_LIST_TYPE_NUM_OF_LISTS; list_number++)
	{
		soli_open_document_selector_store_update_list_async (selector->selector_store,
		                                                      selector,
		                                                      NULL,
	...
	GTK_WIDGET_CLASS (soli_open_document_selector_parent_class)->map (widget);
}

static GtkSizeRequestMode
soli_open_document_selector_get_request_mode (GtkWidget *widget G_GNUC_UNUSED)
{
	return GTK_SIZE_REQUEST_CONSTANT_SIZE;
}

static void
soli_open_document_selector_get_preferred_width (GtkWidget *widget G_GNUC_UNUSED,
                                                  gint      *minimum_width,
                                                  gint      *natural_width)
{
...
static void
soli_open_document_selector_set_property (GObject      *object,
                                           guint         prop_id,
                                           const GValue *value,
                                           GParamSpec   *pspec)
{
...
static void
soli_open_document_selector_get_property (GObject    *object,
                                           guint       prop_id,
                                           GValue     *value,
                                           GParamSpec *pspec)
{
...
static void
soli_open_document_selector_file_activated (SoliOpenDocumentSelector *selector G_GNUC_UNUSED,
...
static void
soli_open_document_selector_class_init (SoliOpenDocumentSelectorClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

	object_class->constructed = soli_open_document_selector_constructed;
	object_class->dispose = soli_open_document_selector_dispose;

	object_class->get_property = soli_open_document_selector_get_property;
	object_class->set_property = soli_open_document_selector_set_property;

	widget_class->get_request_mode = soli_open_document_selector_get_request_mode;
	widget_class->get_preferred_width = soli_open_document_selector_get_preferred_width;
	widget_class->map = soli_open_document_selector_mapped;

	properties[PROP_WINDOW] =
		g_param_spec_object ("window",
		                     "Window",
		                     "The SoliWindow this SoliOpenDocumentSelector is associated with",
		                     SOLI_TYPE_WINDOW,
	...
	signals[SELECTOR_FILE_ACTIVATED] =
		g_signal_new_class_handler ("file-activated",
		                            ...
		                            G_CALLBACK (soli_open_document_selector_file_activated),
		                            NULL, NULL, NULL,
		                            G_TYPE_NONE,
		                            1,
		                            G_TYPE_STRING);
	...
	gtk_widget_class_set_template_from_resource (widget_class,
	                                             "/ca/dluco/soli/ui/soli-open-document-selector.ui");

	gtk_widget_class_bind_template_child (widget_class, SoliOpenDocumentSelector, open_button);
	gtk_widget_class_bind_template_child (widget_class, SoliOpenDocumentSelector, treeview);
	gtk_widget_class_bind_template_child (widget_class, SoliOpenDocumentSelector, placeholder_box);
	gtk_widget_class_bind_template_child (widget_class, SoliOpenDocumentSelector, scrolled_window);
	gtk_widget_class_bind_template_child (widget_class, SoliOpenDocumentSelector, search_entry);
}
...
static void
setup_treeview (SoliOpenDocumentSelector *selector)
{
...
static void
soli_open_document_selector_init (SoliOpenDocumentSelector *selector)
{
	soli_debug (DEBUG_WINDOW);

	gtk_widget_init_template (GTK_WIDGET (selector));

	selector->selector_store = soli_open_document_selector_store_get_default ();
...
SoliOpenDocumentSelector *
soli_open_document_selector_new (SoliWindow *window)
{
	g_return_val_if_fail (SOLI_IS_WINDOW (window), NULL);

	return g_object_new (SOLI_TYPE_OPEN_DOCUMENT_SELECTOR,
	                     "window", window,
	                     NULL);
}

SoliWindow *
soli_open_document_selector_get_window (SoliOpenDocumentSelector *selector)
{
	g_return_val_if_fail (SOLI_IS_OPEN_DOCUMENT_SELECTOR (selector), NULL);

	return selector->window;
}

GtkWidget *
soli_open_document_selector_get_search_entry (SoliOpenDocumentSelector *selector)
{
	g_return_val_if_fail (SOLI_IS_OPEN_DOCUMENT_SELECTOR (selector), NULL);
