...
struct _SoliViewCenteringPrivate
{
	GtkWidget *box;
...
G_DEFINE_TYPE_WITH_PRIVATE (SoliViewCentering, soli_view_centering, GTK_TYPE_BIN)
...
static void
get_spacer_colors (SoliViewCentering   *container,
		   GtkSourceStyleScheme *scheme)
{
	SoliViewCenteringPrivate *priv = container->priv;
...
/* FIXME: when SoliViewCentering will be transfered to GtkSourceView,
 * this method will be replaced by a call to a new method called
 * gtk_source_view_get_right_margin_pixel_position ()
 */
static guint
_soli_view_centering_get_right_margin_pixel_position (SoliViewCentering *container)
{
	...
	g_return_val_if_fail (SOLI_IS_VIEW_CENTERING (container), 0);
...
static void
on_view_right_margin_visibility_changed (SoliView          *view,
					 GParamSpec         *pspec,
					 SoliViewCentering *container)
{
	SoliViewCenteringPrivate *priv = container->priv;
	gboolean visibility;
...
static void
on_view_right_margin_position_changed (SoliView          *view,
				       GParamSpec         *pspec,
				       SoliViewCentering *container)
{
	SoliViewCenteringPrivate *priv = container->priv;
	...
	priv->view_text_width = _soli_view_centering_get_right_margin_pixel_position (container);
...
static gboolean
on_spacer_draw (SoliViewCentering *container,
		cairo_t            *cr,
...
static void
soli_view_centering_remove (GtkContainer *container,
			     GtkWidget    *child)
{
	SoliViewCenteringPrivate *priv;

	g_assert (SOLI_IS_VIEW_CENTERING (container));

	priv = SOLI_VIEW_CENTERING (container)->priv;
	...
	{
		GTK_CONTAINER_CLASS (soli_view_centering_parent_class)->remove (container, child);
	}
}

static void
soli_view_centering_add (GtkContainer *container,
			  GtkWidget    *child)
{
	SoliViewCenteringPrivate *priv;
	GtkTextBuffer *buffer;
	...
	if (SOLI_IS_VIEW (child))
	{
		if (priv->sourceview)
		{
			soli_view_centering_remove (container, priv->sourceview);
		}
		...
		buffer = gtk_text_view_get_buffer (GTK_TEXT_VIEW (priv->sourceview));
		scheme = gtk_source_buffer_get_style_scheme (GTK_SOURCE_BUFFER (buffer));
		get_spacer_colors (SOLI_VIEW_CENTERING (container), scheme);
	...
	{
		GTK_CONTAINER_CLASS (soli_view_centering_parent_class)->add (container, child);
	}
}
...
static void
soli_view_centering_size_allocate (GtkWidget     *widget,
				    GtkAllocation *alloc)
{
	SoliViewCenteringPrivate *priv;
	GtkTextView *view;
	...
	g_assert (SOLI_IS_VIEW_CENTERING (widget));

	priv = SOLI_VIEW_CENTERING (widget)->priv;
	...
	GTK_WIDGET_CLASS (soli_view_centering_parent_class)->size_allocate (widget, alloc);
}

static void
soli_view_centering_finalize (GObject *object)
{
	SoliViewCentering *container = SOLI_VIEW_CENTERING (object);
	SoliViewCenteringPrivate *priv = container->priv;

	if (priv->sourceview)
	{
		soli_view_centering_remove (GTK_CONTAINER (container), priv->sourceview);
	}

	G_OBJECT_CLASS (soli_view_centering_parent_class)->finalize (object);
}

static void
soli_view_centering_class_init (SoliViewCenteringClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
	GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);
	GtkContainerClass *container_class = GTK_CONTAINER_CLASS (klass);

	gobject_class->finalize = soli_view_centering_finalize;

	widget_class->size_allocate = soli_view_centering_size_allocate;

	container_class->add = soli_view_centering_add;
	container_class->remove = soli_view_centering_remove;
}

static void
soli_view_centering_init (SoliViewCentering *container)
{
	SoliViewCenteringPrivate *priv;

	container->priv = soli_view_centering_get_instance_private (container);
	priv = container->priv;
...
/**
 * soli_view_centering_set_centered:
 * @container: a #SoliViewCentering.
 * @centered: whether to center the sourceview child or not.
 *
 * If @centered is %TRUE, the sourceview child is centered
 * horizontally on the #SoliViewCentering container.
 **/
void
soli_view_centering_set_centered (SoliViewCentering *container,
				   gboolean            centered)
{
	g_return_if_fail (SOLI_IS_VIEW_CENTERING (container));

	container->priv->centered = centered != FALSE;

	on_view_right_margin_visibility_changed (SOLI_VIEW (container->priv->sourceview), NULL, container);
}

/**
 * soli_view_centering_get_centered:
 * @container: a #SoliViewCentering.
 *
 ...
 **/
gboolean
soli_view_centering_get_centered (SoliViewCentering *container)
{
	g_return_val_if_fail (SOLI_IS_VIEW_CENTERING (container), FALSE);

	return container->priv->centered;
}

SoliViewCentering *
soli_view_centering_new (void)
{
	return g_object_new (SOLI_TYPE_VIEW_CENTERING,
	                     NULL);
}
