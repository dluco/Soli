...
/* You need to call soli_open_document_selector_store_get_default()
 * to get a singleton #SoliOpenDocumentSelectorStore object.
 * #SoliOpenDocumentSelectorStore is responsible of managing
 * the recent files list and computing others lists.
 *
 * The lists returned are lists of FileItem structs.
 *
 * #SoliOpenDocumentSelectorStore is destroyed automaticly at
 * the end of your application.
 *
 * Call soli_open_document_selector_store_update_list_async() with
 * the corresponding ListType, then in your callback, call
 * soli_open_document_selector_store_update_list_finish() to get
 * in return the list of FileItem structs.
 *
 * The recent files list can be filtered by calling
 * soli_open_document_selector_store_set_filter()
 * and you can get the actual filter by calling
 * soli_open_document_selector_store_get_filter()
 * ( this is in addition to the text mime type filter)
 *
 * The recent files list is not capped by Soli settings like
 * in soli_recent_get_items() but you still can get the limit
 * by calling soli_open_document_selector_store_get_recent_limit().
 *
 * The original setting is stored in gsettings at :
 * ca.dluco.soli.preferences.editor
...
struct _SoliOpenDocumentSelectorStore
{
...
G_DEFINE_TYPE (SoliOpenDocumentSelectorStore, soli_open_document_selector_store, G_TYPE_OBJECT)

G_DEFINE_QUARK (soli-open-document-selector-store-error-quark,
                soli_open_document_selector_store_error)

static GList *
get_current_docs_list (SoliOpenDocumentSelectorStore *selector_store G_GNUC_UNUSED,
                       SoliOpenDocumentSelector      *selector)
{
	...
	window = soli_open_document_selector_get_window (selector);

	docs = soli_window_get_documents (window);
	for (l = docs; l != NULL; l = l->next)
	{
		file = gtk_source_file_get_location (soli_document_get_file (l->data));
		if (file == NULL)
		{
		...
		item = soli_open_document_selector_create_fileitem_item ();
...
static GList *
get_children_from_dir (SoliOpenDocumentSelectorStore *selector_store G_GNUC_UNUSED,
                       GFile                          *dir)
{
	GList *file_items_list = NULL;
...
static GList *
get_active_doc_dir_list (SoliOpenDocumentSelectorStore *selector_store,
                         SoliOpenDocumentSelector      *selector)
{
	SoliWindow *window;
	SoliDocument *active_doc;
	GtkSourceFile *source_file;
	GList *file_items_list = NULL;

	window = soli_open_document_selector_get_window (selector);

	active_doc = soli_window_get_active_document (window);
	...
	source_file = soli_document_get_file (active_doc);
...
static GFile *
get_file_browser_root (SoliOpenDocumentSelectorStore *selector_store G_GNUC_UNUSED,
                       SoliOpenDocumentSelector      *selector)
{
	SoliWindow *window;
	SoliMessageBus *bus;
	...
	bus = soli_window_get_message_bus (window);
	if (soli_message_bus_is_registered (bus, "/plugins/filebrowser", "get_root"))
	{
		msg = soli_message_bus_send_sync (bus, "/plugins/filebrowser", "get_root", NULL, NULL);
...
static GList *
get_file_browser_root_dir_list (SoliOpenDocumentSelectorStore *selector_store,
                                SoliOpenDocumentSelector      *selector)
{
	GFile *root;
	GList *file_items_list = NULL;
...
static GList *
get_local_bookmarks_list (SoliOpenDocumentSelectorStore *selector_store,
                          SoliOpenDocumentSelector      *selector G_GNUC_UNUSED)
{
...
static GList *
get_desktop_dir_list (SoliOpenDocumentSelectorStore *selector_store,
                      SoliOpenDocumentSelector      *selector G_GNUC_UNUSED)
{
	GList *file_items_list = NULL;
...
static GList *
get_home_dir_list (SoliOpenDocumentSelectorStore *selector_store,
                   SoliOpenDocumentSelector      *selector G_GNUC_UNUSED)
{
	GList *file_items_list = NULL;
...
static GList *
get_recent_files_list (SoliOpenDocumentSelectorStore *selector_store,
                       SoliOpenDocumentSelector      *selector G_GNUC_UNUSED)
{
	...
	recent_items_list = soli_recent_get_items (&selector_store->recent_config);
                ...
                gpointer                        user_data G_GNUC_UNUSED)
{
	...
	switch (type)
	{
		case SOLI_OPEN_DOCUMENT_SELECTOR_RECENT_FILES_LIST:
			soli_open_document_selector_free_file_items_list (selector_store->recent_items);
...
on_recent_manager_changed (GtkRecentManager *manager G_GNUC_UNUSED,
                           gpointer          user_data)
{
	SoliOpenDocumentSelectorStore *selector_store = SOLI_OPEN_DOCUMENT_SELECTOR_STORE (user_data);

	selector_store->recent_items_need_update = TRUE;
	soli_open_document_selector_store_update_list_async (selector_store,
	                                                      NULL,
	                                                      NULL,
...
static void
soli_open_document_selector_store_dispose (GObject *object)
{
	SoliOpenDocumentSelectorStore *selector_store = SOLI_OPEN_DOCUMENT_SELECTOR_STORE (object);

	soli_recent_configuration_destroy (&selector_store->recent_config);
	...
	G_OBJECT_CLASS (soli_open_document_selector_store_parent_class)->dispose (object);
}

static void
soli_open_document_selector_store_class_init (SoliOpenDocumentSelectorStoreClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	gobject_class->dispose = soli_open_document_selector_store_dispose;
}

/* The order of functions pointers must be the same as in
 * ListType enum define in ./soli-open-document-selector-helper.h
 */
static GList * (*list_func [])(SoliOpenDocumentSelectorStore *selector_store,
                               SoliOpenDocumentSelector      *selector) =
{
...
update_recent_list (gpointer user_data)
{
	...
	{
		file_items_list = soli_open_document_selector_copy_file_items_list (selector_store->recent_items);
	...
	else
	{
		...
		{
			selector_store->recent_items = soli_open_document_selector_copy_file_items_list (file_items_list);
		}
	}

	g_task_return_pointer (task,
	                       file_items_list,
	                       (GDestroyNotify)soli_open_document_selector_free_file_items_list);
...
{
	SoliOpenDocumentSelectorStore *selector_store = source_object;
	SoliOpenDocumentSelector *selector;
	PushMessage *message;
	ListType type;
	GList *file_items_list;
	...
	if (type >= SOLI_OPEN_DOCUMENT_SELECTOR_LIST_TYPE_NUM_OF_LISTS)
	{
		g_task_return_new_error (task,
		                         SOLI_OPEN_DOCUMENT_SELECTOR_STORE_ERROR, TYPE_OUT_OF_RANGE,
...
GList *
soli_open_document_selector_store_update_list_finish (SoliOpenDocumentSelectorStore  *open_document_selector_store,
                                                       GAsyncResult                    *result,
                                                       GError                         **error)
{
	g_return_val_if_fail (SOLI_IS_OPEN_DOCUMENT_SELECTOR_STORE (open_document_selector_store), NULL);
...
void
soli_open_document_selector_store_update_list_async (SoliOpenDocumentSelectorStore *selector_store,
                                                      SoliOpenDocumentSelector      *selector,
                                                      ...
                                                      gpointer                        user_data)
{
	...
	g_return_if_fail (SOLI_IS_OPEN_DOCUMENT_SELECTOR_STORE (selector_store));
	g_return_if_fail (selector == NULL || SOLI_IS_OPEN_DOCUMENT_SELECTOR (selector));
	...
	g_task_set_source_tag (task, soli_open_document_selector_store_update_list_async);
	g_task_set_priority (task, G_PRIORITY_DEFAULT);
	g_task_set_task_data (task, message, (GDestroyNotify)g_free);

	if (type == SOLI_OPEN_DOCUMENT_SELECTOR_RECENT_FILES_LIST &&
	    selector_store->recent_source == NULL)
	{
...
static void
soli_open_document_selector_store_init (SoliOpenDocumentSelectorStore *selector_store)
{
	soli_recent_configuration_init_default (&selector_store->recent_config);

	/* We remove the recent files limit since we need the whole list but
	 * we back it up as soli_open_document_selector_store_get_recent_limit
	 * use it
	 */
...
gint
soli_open_document_selector_store_get_recent_limit (SoliOpenDocumentSelectorStore *selector_store)
{
	g_return_val_if_fail (SOLI_IS_OPEN_DOCUMENT_SELECTOR_STORE (selector_store), -1);

	return selector_store->recent_config_limit;
}

void
soli_open_document_selector_store_set_filter (SoliOpenDocumentSelectorStore *selector_store,
                                               const gchar                    *filter)
{
	gchar *old_filter;
...
gchar *
soli_open_document_selector_store_get_filter (SoliOpenDocumentSelectorStore *selector_store)
{
	gchar *recent_filter;

	g_return_val_if_fail (SOLI_IS_OPEN_DOCUMENT_SELECTOR_STORE (selector_store), NULL);
...
/* Gets a unique instance of #SoliOpenDocumentSelectorStore
 *
 * Returns: (transfer none): A unique #SoliOpenDocumentSelectorStore.
 * Do not ref or unref it, it will be destroyed at the end of the application.
 */
SoliOpenDocumentSelectorStore *
soli_open_document_selector_store_get_default (void)
{
	static SoliOpenDocumentSelectorStore *instance;

	if (instance == NULL)
	{
		instance = g_object_new (SOLI_TYPE_OPEN_DOCUMENT_SELECTOR_STORE, NULL);
