...
void
soli_open_document_selector_debug_print_list (const gchar *title,
                                               GList       *fileitem_list)
{
	FileItem *item;
	GList *l;
...
FileItem *
soli_open_document_selector_create_fileitem_item (void)
{
	FileItem *item;
...
void
soli_open_document_selector_free_fileitem_item (FileItem *item)
{
	g_free (item->uri);
	g_free (item->name);
	g_free (item->path);
...
FileItem *
soli_open_document_selector_copy_fileitem_item (FileItem *item)
{
	FileItem *new_item;

	new_item = soli_open_document_selector_create_fileitem_item ();
...
inline GList *
soli_open_document_selector_copy_file_items_list (const GList *file_items_list)
{
	GList *new_file_items_list;

	new_file_items_list = g_list_copy_deep ((GList *)file_items_list,
	                                        (GCopyFunc)soli_open_document_selector_copy_fileitem_item,
	                                        NULL);
...
inline void
soli_open_document_selector_free_file_items_list (GList *file_items_list)
{
	g_list_free_full (file_items_list,
	                  (GDestroyNotify)soli_open_document_selector_free_fileitem_item);
}
