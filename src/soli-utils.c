...
void
soli_utils_menu_position_under_widget (GtkMenu  *menu,
					gint     *x,
					gint     *y,
					gboolean *push_in,
					gpointer  user_data)
...
void
soli_utils_menu_position_under_tree_view (GtkMenu  *menu,
					   gint     *x,
					   gint     *y,
					   gboolean *push_in,
					   gpointer  user_data)
{
	...
	else
	{
		...
		soli_utils_menu_position_under_widget (menu,
							x, y, push_in,
							tree);
	}
}
...
/**
 * soli_utils_set_atk_name_description:
 ...
 */
void
soli_utils_set_atk_name_description (GtkWidget   *widget,
				      const gchar *name,
 ...
 */
void
soli_utils_set_atk_relation (GtkWidget       *obj1,
			      GtkWidget       *obj2,
...
void
soli_warning (GtkWindow *parent, const gchar *format, ...)
{
	va_list         args;
	gchar          *str;
	GtkWidget      *dialog;
 ...
 */
gchar *
soli_utils_escape_underscores (const gchar *text,
				gssize       length)
{
	GString *str;
	const gchar *p;
...
static gchar *
soli_utils_str_truncate (const gchar *string,
...
gchar *
soli_utils_str_middle_truncate (const gchar *string,
				 guint        truncate_length)
{
	return soli_utils_str_truncate (string, truncate_length, TRUE);
}

gchar *
soli_utils_str_end_truncate (const gchar *string,
			      guint        truncate_length)
{
	return soli_utils_str_truncate (string, truncate_length, FALSE);
}

gchar *
soli_utils_make_valid_utf8 (const char *name)
{
	GString *string;
...
uri_get_dirname (const gchar *uri)
{
	...
	res = soli_utils_replace_home_dir_with_tilde (str);
 ...
 */
gchar *
soli_utils_uri_get_dirname (const gchar *uri)
{
	return uri_get_dirname (uri);
}

/**
 * soli_utils_location_get_dirname_for_display:
 ...
 */
gchar *
soli_utils_location_get_dirname_for_display (GFile *location)
{
	gchar *uri;
	gchar *res;
	GMount *mount;
...
gchar *
soli_utils_replace_home_dir_with_tilde (const gchar *uri)
{
	gchar *tmp;
	gchar *home;
...
/**
 * soli_utils_get_current_workspace:
 * @screen: a #GdkScreen
 *
 ...
 */
guint
soli_utils_get_current_workspace (GdkScreen *screen)
{
#ifdef GDK_WINDOWING_X11
	GdkWindow *root_win;
	GdkDisplay *display;
...
/**
 * soli_utils_get_window_workspace:
 * @gtkwindow: a #GtkWindow.
 *
 * Get the workspace the window is on.
 *
 * This function gets the workspace that the #GtkWindow is visible on,
 * it returns SOLI_ALL_WORKSPACES if the window is sticky, or if
 * the window manager doesn't support this function.
 *
 * Returns: the workspace the window is on.
 */
guint
soli_utils_get_window_workspace (GtkWindow *gtkwindow)
{
#ifdef GDK_WINDOWING_X11
	GdkWindow *window;
	GdkDisplay *display;
...
/**
 * soli_utils_get_current_viewport:
 * @screen: a #GdkScreen
 * @x: (out): x-axis point.
 * @y: (out): y-axis point.
 *
 ...
 */
void
soli_utils_get_current_viewport (GdkScreen    *screen,
				  gint         *x,
				  gint         *y)
{
#ifdef GDK_WINDOWING_X11
...
gboolean
soli_utils_is_valid_location (GFile *location)
{
	const guchar *p;
	gchar *uri;
	gboolean is_valid;
 ...
 */
gboolean
soli_utils_get_ui_objects (const gchar  *filename,
			    gchar       **root_objects,
			    GtkWidget   **error_widget,
			    const gchar  *object_name,
			    ...)
...
/**
 * soli_utils_get_ui_objects_with_translation_domain:
 * @filename: the path to the gtk builder file
 ...
 */
gboolean
soli_utils_get_ui_objects_with_translation_domain (const gchar  *filename,
                                                    const gchar  *translation_domain,
                                                    gchar       **root_objects,
                                                    GtkWidget   **error_widget,
                                                    const gchar  *object_name,
...
make_canonical_uri_from_shell_arg (const gchar *str)
{
	 ...
	 * <paolo>: I will use soli_utils_is_valid_uri ()
	 *
	 */

	gfile = g_file_new_for_commandline_arg (str);

	if (soli_utils_is_valid_location (gfile))
	{
...
/**
 * soli_utils_make_canonical_uri_from_shell_arg:
 * @str: shell arg.
 *
 ...
 */
gchar *
soli_utils_make_canonical_uri_from_shell_arg (const gchar *str)
{
	return make_canonical_uri_from_shell_arg (str);
}

/**
 * soli_utils_basename_for_display:
 ...
 */
gchar *
soli_utils_basename_for_display (GFile *location)
{
	gchar *name;
	gchar *hn;
	gchar *uri;
	...
	if (g_file_has_uri_scheme (location, "file"))
	{
		...
		else
		{
			...
			g_free (local_path);
	          ...
	          !soli_utils_decode_uri (uri, NULL, NULL, &hn, NULL, NULL))
	{
	...
	else
	{
		...
		if  (hn != NULL)
		{
			hn_utf8 = soli_utils_make_valid_utf8 (hn);
		}
		else
		{
 ...
 */
gchar **
soli_utils_drop_get_uris (GtkSelectionData *selection_data)
{
	gchar **uris;
	gint i;
	gint p = 0;
 ...
 */
gboolean
soli_utils_decode_uri (const gchar  *uri,
			gchar       **scheme,
			gchar       **user,
			gchar       **host,
			gchar       **port,
...
soli_utils_get_compression_type_from_content_type (const gchar *content_type)
{
...
get_direct_save_filename (GdkDragContext *context)
{
	...
	if (*prop_text == '\0' ||
		...
		soli_debug_message (DEBUG_UTILS, "Invalid filename provided by XDS drag site");
		g_free (prop_text);
		return NULL;
	}
...
gchar *
soli_utils_set_direct_save_filename (GdkDragContext *context)
{
	gchar *uri;
	gchar *filename;
	...
	if (filename != NULL)
	{
		...
		tempdir = g_dir_make_tmp ("soli-drop-XXXXXX", NULL);
		if (tempdir == NULL)
		{
...
const gchar *
soli_utils_newline_type_to_string (GtkSourceNewlineType newline_type)
{
	switch (newline_type)
	{
