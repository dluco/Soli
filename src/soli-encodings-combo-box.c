...
G_DEFINE_TYPE (SoliEncodingsComboBox, soli_encodings_combo_box, GTK_TYPE_COMBO_BOX)

static void	update_menu		(SoliEncodingsComboBox       *combo_box);

static void
soli_encodings_combo_box_set_property (GObject    *object,
					guint       prop_id,
					const       GValue *value,
					GParamSpec *pspec)
{
	SoliEncodingsComboBox *combo;

	combo = SOLI_ENCODINGS_COMBO_BOX (object);
...
static void
soli_encodings_combo_box_get_property (GObject    *object,
					guint       prop_id,
					GValue 	   *value,
					GParamSpec *pspec)
{
...
static void
soli_encodings_combo_box_dispose (GObject *object)
{
	SoliEncodingsComboBox *combo = SOLI_ENCODINGS_COMBO_BOX (object);

	g_clear_object (&combo->store);

	G_OBJECT_CLASS (soli_encodings_combo_box_parent_class)->dispose (object);
}

static void
soli_encodings_combo_box_constructed (GObject *object)
{
	SoliEncodingsComboBox *combo = SOLI_ENCODINGS_COMBO_BOX (object);
	GtkCellRenderer *text_renderer;

	G_OBJECT_CLASS (soli_encodings_combo_box_parent_class)->constructed (object);
...
static void
soli_encodings_combo_box_class_init (SoliEncodingsComboBoxClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->set_property = soli_encodings_combo_box_set_property;
	object_class->get_property = soli_encodings_combo_box_get_property;
	object_class->dispose = soli_encodings_combo_box_dispose;
	object_class->constructed = soli_encodings_combo_box_constructed;

	/**
	 * SoliEncodingsComboBox:save-mode:
	 *
...
static void
configure_encodings (SoliEncodingsComboBox *menu)
{
	GtkWidget *dialog;
...
static void
changed_cb (SoliEncodingsComboBox *menu,
	    GtkTreeModel           *model)
{
	GtkTreeIter iter;
...
static void
update_menu (SoliEncodingsComboBox *menu)
{
	GtkListStore *store;
	GtkTreeIter iter;
	GSList *encodings;
	...
	while (encodings)
	{
		...
		gtk_list_store_append (store, &iter);
		gtk_list_store_set (store, &iter,
				    COLUMN_NAME, soli_encoding_item_get_name (item),
				    COLUMN_ENCODING, soli_encoding_item_get_encoding (item),
				    COLUMN_CONFIGURE_ROW, FALSE,
				    -1);

		soli_encoding_item_free (item);
...
static void
soli_encodings_combo_box_init (SoliEncodingsComboBox *menu)
{
 ...
 * provided.
 *
 * Returns: a new #SoliEncodingsComboBox object.
 */
GtkWidget *
soli_encodings_combo_box_new (gboolean save_mode)
{
	return g_object_new (SOLI_TYPE_ENCODINGS_COMBO_BOX,
			     "save_mode", save_mode,
			     NULL);
}

/**
 * soli_encodings_combo_box_get_selected_encoding:
 * @menu: a #SoliEncodingsComboBox.
 *
...
soli_encodings_combo_box_get_selected_encoding (SoliEncodingsComboBox *menu)
{
	GtkTreeIter iter;

	g_return_val_if_fail (SOLI_IS_ENCODINGS_COMBO_BOX (menu), NULL);
...
/**
 * soli_encodings_combo_box_set_selected_encoding:
 * @menu: a #SoliEncodingsComboBox.
 ...
 */
void
soli_encodings_combo_box_set_selected_encoding (SoliEncodingsComboBox  *menu,
						 const GtkSourceEncoding *encoding)
{
	...
	g_return_if_fail (SOLI_IS_ENCODINGS_COMBO_BOX (menu));
