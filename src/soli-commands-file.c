...
#define SOLI_OPEN_DIALOG_KEY "soli-open-dialog-key"
#define SOLI_IS_CLOSING_ALL "soli-is-closing-all"
#define SOLI_NOTEBOOK_TO_CLOSE "soli-notebook-to-close"
#define SOLI_IS_QUITTING "soli-is-quitting"
#define SOLI_IS_QUITTING_ALL "soli-is-quitting-all"

static void tab_state_changed_while_saving (SoliTab    *tab,
					    GParamSpec  *pspec,
					    SoliWindow *window);

void
_soli_cmd_file_new (GSimpleAction *action,
                     GVariant      *parameter,
                     gpointer       user_data)
{
...
/* File loading */
static GSList *
load_file_list (SoliWindow             *window,
		const GSList            *files,
		...
		gboolean                 create)
{
	...
	win_docs = soli_window_get_documents (window);
	...
	for (l = files; l != NULL; l = l->next)
	{
		...
		else
		{
			if (l == files)
			{
				...
				soli_window_set_active_tab (window, tab);
				jump_to = FALSE;
				...
				if (line_pos > 0)
				{
					...
					{
						soli_document_goto_line (doc, line_pos - 1);
					}

					soli_view_scroll_to_cursor (soli_tab_get_view (tab));
				}
			}

			++num_loaded_files;
			loaded_files = g_slist_prepend (loaded_files,
			                                soli_tab_get_document (tab));
		}
	}
	...
	if (tab != NULL)
	{
		...
		if (soli_document_is_untouched (doc) &&
		    soli_tab_get_state (tab) == SOLI_TAB_STATE_NORMAL)
		{
			_soli_tab_load (tab,
					 l->data,
					 encoding,
			...
			gtk_widget_grab_focus (GTK_WIDGET (soli_tab_get_view (tab)));
	...
	while (l != NULL)
	{
		...
		tab = soli_window_create_tab_from_location (window,
							     l->data,
							     encoding,
							     line_pos,
							     column_pos,
	...
	if (num_loaded_files == 1)
	{
		...
		doc = soli_tab_get_document (tab);
		uri_for_display = soli_document_get_uri_for_display (doc);
 ...
 */
void
soli_commands_load_location (SoliWindow             *window,
			      GFile                   *location,
			      const GtkSourceEncoding *encoding,
			      gint                     line_pos,
			      gint                     column_pos)
{
	...
	g_return_if_fail (soli_utils_is_valid_location (location));

	uri = g_file_get_uri (location);
	soli_debug_message (DEBUG_COMMANDS, "Loading URI '%s'", uri);
	g_free (uri);
 ...
 *
 * Returns: (element-type Soli.Document) (transfer container): the locations
 * that were loaded.
 */
GSList *
soli_commands_load_locations (SoliWindow             *window,
 ...
 */
GSList *
_soli_cmd_load_files_from_prompt (SoliWindow             *window,
				   GSList                  *files,
				   const GtkSourceEncoding *encoding,
				   gint                     line_pos,
				   gint                     column_pos)
...
static void
open_dialog_destroyed (SoliWindow            *window,
		       SoliFileChooserDialog *dialog)
{
	soli_debug (DEBUG_COMMANDS);

	g_object_set_data (G_OBJECT (window),
			   SOLI_OPEN_DIALOG_KEY,
			   NULL);
}

static void
open_dialog_response_cb (SoliFileChooserDialog *dialog,
                         gint                    response_id,
                         SoliWindow            *window)
{
	GSList *files;
	...
	{
		soli_file_chooser_dialog_destroy (dialog);
		return;
	}

	files = soli_file_chooser_dialog_get_files (dialog);
	g_return_if_fail (files != NULL);

	encoding = soli_file_chooser_dialog_get_encoding (dialog);

	soli_file_chooser_dialog_destroy (dialog);

	if (window == NULL)
	{
		window = soli_app_create_window (SOLI_APP (g_application_get_default ()),
		                                  NULL);
	...
	_soli_window_set_default_location (window, files->data);
...
void
_soli_cmd_file_open (GSimpleAction *action,
                      GVariant      *parameter,
                      gpointer       user_data)
{
	SoliWindow *window = NULL;
	SoliFileChooserDialog *open_dialog;

	if (SOLI_IS_WINDOW (user_data))
	{
		window = user_data;
	}
	...
	if (window != NULL)
	{
		...
		data = g_object_get_data (G_OBJECT (window), SOLI_OPEN_DIALOG_KEY);

		if (data != NULL)
		{
			g_return_if_fail (SOLI_IS_FILE_CHOOSER_DIALOG (data));
		...
		/* FIXME: this should be in SoliWindow. */
	...
	open_dialog = soli_file_chooser_dialog_create (C_("window title", "Open"),
							window != NULL ? GTK_WINDOW (window) : NULL,
							SOLI_FILE_CHOOSER_OPEN |
							SOLI_FILE_CHOOSER_ENABLE_ENCODING |
							SOLI_FILE_CHOOSER_ENABLE_DEFAULT_FILTERS,
							NULL,
	...
	if (window != NULL)
	{
		...
		{
			default_path = _soli_window_get_default_location (window);
		}

		if (default_path != NULL)
		{
			soli_file_chooser_dialog_set_current_folder (open_dialog, default_path);
	...
	soli_file_chooser_dialog_show (open_dialog);
}

void
_soli_cmd_file_reopen_closed_tab (GSimpleAction *action,
				   GVariant      *parameter,
				   gpointer       user_data)
{
	SoliWindow *window = SOLI_WINDOW (user_data);
	...
	file = _soli_window_pop_last_closed_doc (window);
	if (file != NULL)
	{
		soli_commands_load_location (window, file, NULL, 0, 0);
	}
}
			...
			GFile     *file)
{
	 ...
	 */
	name_for_display = soli_utils_str_middle_truncate (parse_name, 50);
	g_free (parse_name);
...
get_compression_type_from_file (GFile *file)
{
	...
	type = soli_utils_get_compression_type_from_content_type (content_type);
...
tab_save_as_ready_cb (SoliTab     *tab,
		      ...
		      GTask        *task)
{
	gboolean success = _soli_tab_save_finish (tab, result);
	g_task_return_boolean (task, success);
	g_object_unref (task);
}

static void
save_dialog_response_cb (SoliFileChooserDialog *dialog,
			 gint                    response_id,
			 GTask                  *task)
{
	SoliTab *tab;
	...
	location = soli_file_chooser_dialog_get_file (dialog);
	...
	{
		GtkWindow *dialog_window = soli_file_chooser_dialog_get_window (dialog);
	...
	encoding = soli_file_chooser_dialog_get_encoding (dialog);
	newline_type = soli_file_chooser_dialog_get_newline_type (dialog);
	...
	_soli_window_set_default_location (window, location);

	_soli_tab_save_as_async (tab,
				  location,
				  encoding,
				  newline_type,
				  compression_type,
...
confirm_overwrite_callback (SoliFileChooserDialog *dialog,
			    gpointer                data)
{
	...
	file = soli_file_chooser_dialog_get_file (dialog);
	...
	if (info != NULL)
	{
		    ...
		    !g_file_info_get_attribute_boolean (info, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE))
		{
			...
			win = soli_file_chooser_dialog_get_window (dialog);
...
{
	GTask *task;
	SoliFileChooserDialog *save_dialog;
	...
	save_dialog = soli_file_chooser_dialog_create (C_("window title", "Save As"),
							GTK_WINDOW (window),
							SOLI_FILE_CHOOSER_SAVE |
							SOLI_FILE_CHOOSER_ENABLE_ENCODING |
							SOLI_FILE_CHOOSER_ENABLE_LINE_ENDING |
							SOLI_FILE_CHOOSER_ENABLE_DEFAULT_FILTERS,
							NULL,
							_("_Cancel"), GTK_RESPONSE_CANCEL,
							_("_Save"), GTK_RESPONSE_OK);

	soli_file_chooser_dialog_set_do_overwrite_confirmation (save_dialog, TRUE);
	...
	window_group = soli_window_get_group (window);

	dialog_window = soli_file_chooser_dialog_get_window (save_dialog);
	...
	soli_file_chooser_dialog_set_modal (save_dialog, TRUE);
	...
	if (location != NULL)
	{
		soli_file_chooser_dialog_set_file (save_dialog, location);
	}
	else
	{
		GFile *default_path;
		...
		default_path = _soli_window_get_default_location (window);
		docname = soli_document_get_short_name_for_display (doc);

		if (default_path != NULL)
		{
			soli_file_chooser_dialog_set_current_folder (save_dialog,
								      default_path);

			g_object_unref (default_path);
		}

		soli_file_chooser_dialog_set_current_name (save_dialog,
							    docname);
	...
	soli_file_chooser_dialog_set_encoding (SOLI_FILE_CHOOSER_DIALOG (save_dialog),
						encoding);

	soli_file_chooser_dialog_set_newline_type (SOLI_FILE_CHOOSER_DIALOG (save_dialog),
	                                            newline_type);
	...
	soli_file_chooser_dialog_show (save_dialog);
}
...
/**
 * soli_commands_save_document_async:
 * @document: the #SoliDocument to save.
 * @window: a #SoliWindow.
 ...
 * soli_commands_save_document_finish() to get the result of the operation.
 *
 * Since: 3.14
 */
void
soli_commands_save_document_async (SoliDocument       *document,
				    SoliWindow         *window,
				    GCancellable        *cancellable,
				    GAsyncReadyCallback  callback,
				    gpointer             user_data)
{
	...
	uri_for_display = soli_document_get_uri_for_display (document);
	soli_statusbar_flash_message (SOLI_STATUSBAR (window->priv->statusbar),
				       window->priv->generic_message_cid,
				       _("Saving file '%s'\342\200\246"),
				       uri_for_display);
 ...
 */
gboolean
soli_commands_save_document_finish (SoliDocument *document,
				     GAsyncResult  *result)
{
...
static void
save_tab (SoliTab    *tab,
	  SoliWindow *window)
{
	SoliDocument *doc = soli_tab_get_document (tab);

	soli_commands_save_document_async (doc,
					    window,
					    NULL,
...
void
_soli_cmd_file_save (GSimpleAction *action,
                      GVariant      *parameter,
                      gpointer       user_data)
{
	SoliWindow *window = SOLI_WINDOW (user_data);
...
static void
_soli_cmd_file_save_as_cb (SoliTab     *tab,
			    GAsyncResult *result,
			    gpointer      user_data)
{
...
void
_soli_cmd_file_save_as (GSimpleAction *action,
                         GVariant      *parameter,
                         gpointer       user_data)
{
	SoliWindow *window = SOLI_WINDOW (user_data);
	...
	if (tab != NULL)
	{
		save_as_tab_async (tab,
				   window,
				   NULL,
				   (GAsyncReadyCallback) _soli_cmd_file_save_as_cb,
				   NULL);
	}
}
...
really_close_tab (SoliTab *tab)
{
	...
	g_return_val_if_fail (soli_tab_get_state (tab) == SOLI_TAB_STATE_CLOSING,
			      FALSE);

	toplevel = gtk_widget_get_toplevel (GTK_WIDGET (tab));
	g_return_val_if_fail (SOLI_IS_WINDOW (toplevel), FALSE);

	window = SOLI_WINDOW (toplevel);

	soli_window_close_tab (window, tab);

	if (soli_window_get_active_tab (window) == NULL)
	{
...
close_tab (SoliTab *tab)
{
	 ...
	 */
	_soli_tab_mark_for_closing (tab);
...
static void
save_as_documents_list_cb (SoliTab     *tab,
			   GAsyncResult *result,
			   SaveAsData   *data)
{
	...
	g_return_if_fail (tab == SOLI_TAB (data->tabs_to_save_as->data));
...
{
	SoliTab *next_tab = SOLI_TAB (data->tabs_to_save_as->data);

	soli_window_set_active_tab (data->window, next_tab);
...
/*
 * The docs in the list must belong to the same SoliWindow.
 */
static void
save_documents_list (SoliWindow *window,
		     GList       *docs)
{
	...
	g_return_if_fail ((soli_window_get_state (window) & SOLI_WINDOW_STATE_PRINTING) == 0);
	...
	for (l = docs; l != NULL; l = l->next)
	{
		...
		g_return_if_fail (SOLI_IS_DOCUMENT (l->data));
		doc = l->data;

		tab = soli_tab_get_from_document (doc);
		state = soli_tab_get_state (tab);

		g_return_if_fail (state != SOLI_TAB_STATE_PRINTING);
		g_return_if_fail (state != SOLI_TAB_STATE_CLOSING);

		if (state == SOLI_TAB_STATE_NORMAL ||
		    state == SOLI_TAB_STATE_SHOWING_PRINT_PREVIEW)
		{
			if (_soli_document_needs_saving (doc))
			{
				...
				/* FIXME: manage the case of local readonly files owned by the
				   user is running soli - Paolo (Dec. 8, 2005) */
		...
		{
			/* If the state is:
			   - SOLI_TAB_STATE_LOADING: we do not save since we are sure the file is unmodified
			   - SOLI_TAB_STATE_REVERTING: we do not save since the user wants
			     to return back to the version of the file she previously saved
			   - SOLI_TAB_STATE_SAVING: well, we are already saving (no need to save again)
			   - SOLI_TAB_STATE_PRINTING: there is not a
			   ...
			   - SOLI_TAB_STATE_GENERIC_ERROR: we do not save since the document contains
			     errors (I don't think this is a very frequent case, we should probably remove
			     this state)
			   - SOLI_TAB_STATE_LOADING_ERROR: there is nothing to save
			   - SOLI_TAB_STATE_REVERTING_ERROR: there is nothing to save and saving the current
			     document will overwrite the copy of the file the user wants to go back to
			   - SOLI_TAB_STATE_SAVING_ERROR: we do not save since we just failed to save, so there is
			     no reason to automatically retry... we wait for user intervention
			   - SOLI_TAB_STATE_CLOSING: this state is invalid in this case
			*/

			gchar *uri_for_display;

			uri_for_display = soli_document_get_uri_for_display (doc);
			soli_debug_message (DEBUG_COMMANDS,
...
/**
 * soli_commands_save_all_documents:
 * @window: a #SoliWindow.
 *
 ...
 */
void
soli_commands_save_all_documents (SoliWindow *window)
{
	GList *docs;
	...
	docs = soli_window_get_documents (window);
...
void
_soli_cmd_file_save_all (GSimpleAction *action,
                          GVariant      *parameter,
                          gpointer       user_data)
{
	soli_commands_save_all_documents (SOLI_WINDOW (user_data));
}

/**
 * soli_commands_save_document:
 * @window: a #SoliWindow.
 * @document: the #SoliDocument to save.
 *
 * Asynchronously save @document. @document must belong to @window. If you need
 * the result of the operation, use soli_commands_save_document_async().
 */
void
soli_commands_save_document (SoliWindow   *window,
...
static GtkWidget *
revert_dialog (SoliWindow   *window,
	       SoliDocument *doc)
{
	GtkWidget *dialog;
	gchar *docname;
	...
	seconds = MAX (1, _soli_document_get_seconds_since_last_save_or_load (doc));
...
void
_soli_cmd_file_revert (GSimpleAction *action,
                        GVariant      *parameter,
                        gpointer       user_data)
{
	SoliWindow *window = SOLI_WINDOW (user_data);
	 ...
	 * the user further.
	 */
	if (soli_tab_get_state (tab) == SOLI_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION ||
	    _soli_tab_get_can_close (tab))
	{
		do_revert (window, tab);
		return;
	}
...
static void
tab_state_changed_while_saving (SoliTab    *tab,
				GParamSpec  *pspec,
				SoliWindow *window)
{
	SoliTabState state;
	...
	soli_debug_message (DEBUG_COMMANDS, "State while saving: %d\n", state);
                          ...
                          SoliNotebook *notebook)
{
	...
	for (l = tabs; l != NULL; l = l->next)
	{
		...
		/* If the state is: ([*] invalid states)
		   - SOLI_TAB_STATE_NORMAL: close (and if needed save)
		   - SOLI_TAB_STATE_LOADING: close, we are sure the file is unmodified
		   - SOLI_TAB_STATE_REVERTING: since the user wants
		   ...
		   - [*] SOLI_TAB_STATE_SAVING: invalid, ClosAll
		   ...
		   - [*] SOLI_TAB_STATE_PRINTING: there is not a
		     ...
		     and Quit are unsensitive if the window state is PRINTING.
		   - SOLI_TAB_STATE_SHOWING_PRINT_PREVIEW: close (and if needed save)
		   - SOLI_TAB_STATE_LOADING_ERROR: close without saving (if the state is LOADING_ERROR then the
		     document is not modified)
		   - SOLI_TAB_STATE_REVERTING_ERROR: we do not close since the document contains errors
		   - SOLI_TAB_STATE_SAVING_ERROR: we do not close since the document contains errors
		   - SOLI_TAB_STATE_GENERIC_ERROR: we do not close since the document contains
		...
		*/
		...
		g_return_if_fail (state != SOLI_TAB_STATE_PRINTING);
		g_return_if_fail (state != SOLI_TAB_STATE_CLOSING);
		g_return_if_fail (state != SOLI_TAB_STATE_SAVING);

		if (state != SOLI_TAB_STATE_SAVING_ERROR &&
		    state != SOLI_TAB_STATE_GENERIC_ERROR &&
		    state != SOLI_TAB_STATE_REVERTING_ERROR)
		{
			if (g_list_index (docs, doc) >= 0 &&
			    state != SOLI_TAB_STATE_LOADING &&
			    state != SOLI_TAB_STATE_LOADING_ERROR &&
			    state != SOLI_TAB_STATE_REVERTING) /* FIXME: is this the right behavior with REVERTING ?*/
			{
				GtkSourceFile *file = soli_document_get_file (doc);

				/* The document must be saved before closing */
				g_return_if_fail (_soli_document_needs_saving (doc));
	...
	soli_window_close_tabs (window, tabs_to_close);
	...
	{
		save_and_close (SOLI_TAB (sl->data), window);
	}
			 ...
			 SoliWindow *window)
{
	...
	tab = soli_tab_get_from_document (SOLI_DOCUMENT (docs->data));
	g_return_if_fail (tab != NULL);
...
static void
close_all_tabs (SoliWindow *window)
{
	soli_debug (DEBUG_COMMANDS);
...
static void
close_document (SoliWindow   *window,
		SoliDocument *doc)
{
	SoliTab *tab;
...
static void
close_confirmation_dialog_response_handler (SoliCloseConfirmationDialog *dlg,
                                            gint                          response_id,
                                            SoliWindow                  *window)
{
	GList *selected_documents;
	gboolean is_closing_all;
	SoliNotebook *notebook_to_close;
	...
	notebook_to_close = g_object_get_data (G_OBJECT (window), SOLI_NOTEBOOK_TO_CLOSE);
	...
	switch (response_id)
	{
		...
		case GTK_RESPONSE_YES:
			selected_documents = soli_close_confirmation_dialog_get_selected_documents (dlg);
			...
			if (selected_documents == NULL)
			{
				if (is_closing_all)
				{
					 ...
					 * because close_all_tabs could destroy the soli window */
				...
				{
					soli_notebook_remove_all_tabs (notebook_to_close);
				}
				else
				{
					g_return_if_reached ();
			...
			else
			{
				...
				{
					SoliNotebook *notebook = is_closing_all ? NULL : notebook_to_close;
		...
		case GTK_RESPONSE_NO:
			...
			else
			{
				...
				unsaved_documents = soli_close_confirmation_dialog_get_unsaved_documents (dlg);
				g_return_if_fail (unsaved_documents->next == NULL);

				close_document (window, SOLI_DOCUMENT (unsaved_documents->data));
			}
	...
	g_object_set_data (G_OBJECT (window), SOLI_NOTEBOOK_TO_CLOSE, NULL);
	       ...
	       GtkWindow *window)
{
	...
	if (!_soli_tab_get_can_close (tab))
	{
		...
		dlg = soli_close_confirmation_dialog_new_single (window, doc);
 ...
 * soli_window_close_tab always run the confirm dialog?
 * we should not allow closing a tab without resetting the
 * SOLI_IS_CLOSING_ALL flag!
 */
void
_soli_cmd_file_close_tab (SoliTab    *tab,
			   SoliWindow *window)
{
	soli_debug (DEBUG_COMMANDS);
...
void
_soli_cmd_file_close (GSimpleAction *action,
                       GVariant      *parameter,
                       gpointer       user_data)
{
	SoliWindow *window = SOLI_WINDOW (user_data);
	...
	_soli_cmd_file_close_tab (active_tab, window);
}

static void
file_close_dialog (SoliWindow *window,
                   GList       *unsaved_docs)
{
	GtkWidget *dlg;
	...
	if (unsaved_docs->next == NULL)
	{
		...
		doc = SOLI_DOCUMENT (unsaved_docs->data);
		...
		dlg = soli_close_confirmation_dialog_new_single (GTK_WINDOW (window), doc);
	}
	else
	{
		dlg = soli_close_confirmation_dialog_new (GTK_WINDOW (window), unsaved_docs);
	}
...
static GList *
notebook_get_unsaved_documents (SoliNotebook *notebook)
{
	GList *children;
...
/* Close a notebook */
void
_soli_cmd_file_close_notebook (SoliWindow   *window,
                                SoliNotebook *notebook)
{
	GList *unsaved_docs;

	g_object_set_data (G_OBJECT (window), SOLI_IS_CLOSING_ALL, GBOOLEAN_TO_POINTER (FALSE));
	g_object_set_data (G_OBJECT (window), SOLI_IS_QUITTING, GBOOLEAN_TO_POINTER (FALSE));
	g_object_set_data (G_OBJECT (window), SOLI_IS_QUITTING_ALL, GBOOLEAN_TO_POINTER (FALSE));

	g_object_set_data (G_OBJECT (window), SOLI_NOTEBOOK_TO_CLOSE, notebook);
	...
	if (unsaved_docs == NULL)
	{
		...
		soli_notebook_remove_all_tabs (SOLI_NOTEBOOK (notebook));
	}
	else
	{
                ...
                gboolean     is_quitting)
{
	...
	g_return_if_fail (!(soli_window_get_state (window) &
	                    (SOLI_WINDOW_STATE_SAVING |
	                     SOLI_WINDOW_STATE_PRINTING)));
	...
	unsaved_docs = soli_window_get_unsaved_documents (window);
...
void
_soli_cmd_file_close_all (GSimpleAction *action,
                           GVariant      *parameter,
                           gpointer       user_data)
{
	SoliWindow *window = SOLI_WINDOW (user_data);
...
quit_all (void)
{
	...
	for (l = windows; l != NULL; l = g_list_next (l))
	{
		...
		if (!(soli_window_get_state (window) &
...
void
_soli_cmd_file_quit (GSimpleAction *action,
                      GVariant      *parameter,
                      gpointer       user_data)
{
	SoliWindow *window = SOLI_WINDOW (user_data);
