...
/*
 * soli-preferences dialog is a singleton since we don't
 * want two dialogs showing an inconsistent state of the
 * preferences.
 * When soli_show_preferences_dialog is called and there
...
#define SOLI_SCHEME_ROW_ID_KEY "soli-scheme-row-id"

#define SOLI_TYPE_PREFERENCES_DIALOG (soli_preferences_dialog_get_type())

G_DECLARE_FINAL_TYPE (SoliPreferencesDialog, soli_preferences_dialog, SOLI, PREFERENCES_DIALOG, GtkWindow)
...
G_DEFINE_TYPE (SoliPreferencesDialog, soli_preferences_dialog, GTK_TYPE_WINDOW)

static void
soli_preferences_dialog_dispose (GObject *object)
{
	SoliPreferencesDialog *dlg = SOLI_PREFERENCES_DIALOG (object);

	g_clear_object (&dlg->editor);
	g_clear_object (&dlg->uisettings);

	G_OBJECT_CLASS (soli_preferences_dialog_parent_class)->dispose (object);
}

static void
soli_preferences_dialog_close (SoliPreferencesDialog *dialog)
{
	gtk_window_close (GTK_WINDOW (dialog));
}

static void
soli_preferences_dialog_class_init (SoliPreferencesDialogClass *klass)
{
	...
	object_class->dispose = soli_preferences_dialog_dispose;

	signals[CLOSE] =
		g_signal_new_class_handler ("close",
		                            G_TYPE_FROM_CLASS (klass),
		                            G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		                            G_CALLBACK (soli_preferences_dialog_close),
		                            NULL, NULL, NULL,
		                            G_TYPE_NONE,
		                            0);
	...
	gtk_widget_class_set_template_from_resource (widget_class,
	                                             "/ca/dluco/soli/ui/soli-preferences-dialog.ui");
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, notebook);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, display_line_numbers_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, display_statusbar_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, display_grid_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, display_overview_map_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, right_margin_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, right_margin_position_grid);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, right_margin_position_spinbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, highlight_current_line_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, bracket_matching_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, wrap_text_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, split_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, tabs_width_spinbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, insert_spaces_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, auto_indent_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, backup_copy_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, auto_save_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, auto_save_spinbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, default_font_checkbutton);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, font_button);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, font_grid);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, schemes_list);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, schemes_scrolled_window);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, install_scheme_button);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, uninstall_scheme_button);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, schemes_toolbar);
	gtk_widget_class_bind_template_child (widget_class, SoliPreferencesDialog, plugin_manager);
}

static void
setup_editor_page (SoliPreferencesDialog *dlg)
{
	soli_debug (DEBUG_PREFS);
	...
	g_settings_bind (dlg->editor,
			 ...
			 G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
	g_settings_bind (dlg->editor,
			 SOLI_SETTINGS_AUTO_SAVE_INTERVAL,
...
static void
setup_view_page (SoliPreferencesDialog *dlg)
{
	...
	g_settings_get (dlg->editor, SOLI_SETTINGS_RIGHT_MARGIN_POSITION,
	...
	g_settings_bind (dlg->uisettings,
	                 SOLI_SETTINGS_STATUSBAR_VISIBLE,
	                 dlg->display_statusbar_checkbutton,
	                 "active",
	                 G_SETTINGS_BIND_GET | G_SETTINGS_BIND_SET);
	g_settings_bind (dlg->editor,
			 SOLI_SETTINGS_DISPLAY_OVERVIEW_MAP,
...
static void
setup_font_colors_page_font_section (SoliPreferencesDialog *dlg)
{
	...
	system_font = soli_settings_get_system_font (settings);
...
static void
set_buttons_sensisitivity_according_to_scheme (SoliPreferencesDialog *dlg,
                                               GtkSourceStyleScheme   *scheme)
{
	...
	if (scheme != NULL)
	{
		...
		if (filename != NULL)
		{
			editable = g_str_has_prefix (filename, soli_dirs_get_user_styles_dir ());
		}
	}
                      ...
                      SoliPreferencesDialog      *dlg)
{
	...
	g_settings_set_string (dlg->editor, SOLI_SETTINGS_SCHEME, id);
	set_buttons_sensisitivity_according_to_scheme (dlg, scheme);
}

static GtkSourceStyleScheme *
get_default_color_scheme (SoliPreferencesDialog *dlg)
{
 ...
 * This function copies @fname in #SOLI_STYLES_DIR and ask the style manager to
...
install_style_scheme (const gchar *fname)
{
	...
	styles_dir = soli_dirs_get_user_styles_dir ();
	...
	if (strcmp (dirname, styles_dir) != 0)
	{
		...
		/* Copy the style scheme file into SOLI_STYLES_DIR */
...
static void
add_scheme_chooser_response_cb (SoliFileChooserDialog *chooser,
				gint                    res_id,
				SoliPreferencesDialog *dlg)
{
	GFile *file;
	...
	{
		soli_file_chooser_dialog_hide (chooser);
		return;
	}

	file = soli_file_chooser_dialog_get_file (chooser);
	...
	if (scheme == NULL)
	{
		soli_warning (GTK_WINDOW (dlg),
		               _("The selected color scheme cannot be installed."));

		return;
	}

	g_settings_set_string (dlg->editor, SOLI_SETTINGS_SCHEME,
...
install_scheme_clicked (GtkButton              *button,
			SoliPreferencesDialog *dlg)
{
	SoliFileChooserDialog *chooser;

	if (dlg->install_scheme_file_schooser != NULL)
	{
		soli_file_chooser_dialog_show (dlg->install_scheme_file_schooser);
		return;
	}

	chooser = soli_file_chooser_dialog_create (_("Add Scheme"),
						    GTK_WINDOW (dlg),
	...
	/* Filters */
	soli_file_chooser_dialog_add_pattern_filter (chooser,
	...
	soli_file_chooser_dialog_show (chooser);
}
...
static void
setup_font_colors_page_style_scheme_section (SoliPreferencesDialog *dlg)
{
...
static void
setup_font_colors_page (SoliPreferencesDialog *dlg)
{
...
static void
setup_plugins_page (SoliPreferencesDialog *dlg)
{
	gtk_widget_show_all (dlg->plugin_manager);
}

static void
soli_preferences_dialog_init (SoliPreferencesDialog *dlg)
{
	soli_debug (DEBUG_PREFS);

	dlg->editor = g_settings_new ("ca.dluco.soli.preferences.editor");
	dlg->uisettings = g_settings_new ("ca.dluco.soli.preferences.editor");
...
void
soli_show_preferences_dialog (SoliWindow *parent)
{
	soli_debug (DEBUG_PREFS);

	if (preferences_dialog == NULL)
	{
		preferences_dialog = GTK_WIDGET (g_object_new (SOLI_TYPE_PREFERENCES_DIALOG,
