...
G_DEFINE_TYPE (SoliMenuStackSwitcher, soli_menu_stack_switcher, GTK_TYPE_MENU_BUTTON)

static void
soli_menu_stack_switcher_init (SoliMenuStackSwitcher *switcher)
{
  GtkWidget *box;
  GtkWidget *arrow;
  ...
  gtk_style_context_add_class (context, "soli-menu-stack-switcher");
...
static void
clear_popover (SoliMenuStackSwitcher *switcher)
{
...
on_button_clicked (GtkWidget              *widget,
                   SoliMenuStackSwitcher *switcher)
{
  GtkWidget *child;
...
static void
update_button (SoliMenuStackSwitcher *switcher,
               GtkWidget              *widget,
               GtkWidget              *button)
{
  GList *children;
...
static void
add_child (SoliMenuStackSwitcher *switcher,
           GtkWidget              *widget)
{
  GtkWidget *button;
  GList *group;
...
static void
populate_popover (SoliMenuStackSwitcher *switcher)
{
...
static void
disconnect_stack_signals (SoliMenuStackSwitcher *switcher)
{
...
static void
connect_stack_signals (SoliMenuStackSwitcher *switcher)
{
...
void
soli_menu_stack_switcher_set_stack (SoliMenuStackSwitcher *switcher,
                                     GtkStack               *stack)
{
  g_return_if_fail (SOLI_IS_MENU_STACK_SWITCHER (switcher));
...
GtkStack *
soli_menu_stack_switcher_get_stack (SoliMenuStackSwitcher *switcher)
{
  g_return_val_if_fail (SOLI_IS_MENU_STACK_SWITCHER (switcher), NULL);

  return switcher->stack;
}

static void
soli_menu_stack_switcher_get_property (GObject    *object,
                                        guint       prop_id,
                                        GValue     *value,
                                        GParamSpec *pspec)
{
  SoliMenuStackSwitcher *switcher = SOLI_MENU_STACK_SWITCHER (object);
...
static void
soli_menu_stack_switcher_set_property (GObject      *object,
                                        guint         prop_id,
                                        const GValue *value,
                                        GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_STACK:
      soli_menu_stack_switcher_set_stack (switcher, g_value_get_object (value));
      break;
...
static void
soli_menu_stack_switcher_dispose (GObject *object)
{
  SoliMenuStackSwitcher *switcher = SOLI_MENU_STACK_SWITCHER (object);

  soli_menu_stack_switcher_set_stack (switcher, NULL);

  G_OBJECT_CLASS (soli_menu_stack_switcher_parent_class)->dispose (object);
}

static void
soli_menu_stack_switcher_finalize (GObject *object)
{
  SoliMenuStackSwitcher *switcher = SOLI_MENU_STACK_SWITCHER (object);

  g_hash_table_destroy (switcher->buttons);

  G_OBJECT_CLASS (soli_menu_stack_switcher_parent_class)->finalize (object);
}

static void
soli_menu_stack_switcher_class_init (SoliMenuStackSwitcherClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->get_property = soli_menu_stack_switcher_get_property;
  object_class->set_property = soli_menu_stack_switcher_set_property;
  object_class->dispose = soli_menu_stack_switcher_dispose;
  object_class->finalize = soli_menu_stack_switcher_finalize;
...
GtkWidget *
soli_menu_stack_switcher_new (void)
{
  return g_object_new (SOLI_TYPE_MENU_STACK_SWITCHER, NULL);
}
