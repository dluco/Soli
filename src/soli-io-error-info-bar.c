	...
	switch (code)
	{
		...
		case G_IO_ERROR_HOST_NOT_FOUND:
			 ...
			 */
			{
				...
				if (uri && soli_utils_decode_uri (uri, NULL, NULL, &hn, NULL, NULL))
				{
					if (hn != NULL)
					{
						gchar *host_markup;
						...
						host_name = soli_utils_make_valid_utf8 (hn);
						g_free (hn);
...
GtkWidget *
soli_unrecoverable_reverting_error_info_bar_new (GFile        *location,
						  const GError *error)
{
	 ...
	 */
	temp_uri_for_display = soli_utils_str_middle_truncate (full_formatted_uri,
		  ...
		  GtkWidget *vbox)
{
	...
	menu = soli_encodings_combo_box_new (TRUE);
	g_object_set_data (G_OBJECT (info_bar),
			   "soli-info-bar-encoding-menu",
			   menu);
...
GtkWidget *
soli_network_unavailable_info_bar_new (GFile *location)
{
	GtkWidget *info_bar;
...
GtkWidget *
soli_io_loading_error_info_bar_new (GFile                   *location,
...
GtkWidget *
soli_conversion_error_while_saving_info_bar_new (GFile                   *location,
						  const GtkSourceEncoding *encoding,
						  const GError            *error)
{
	gchar *error_message = NULL;
...
soli_conversion_error_info_bar_get_encoding (GtkWidget *info_bar)
{
	gpointer menu;
	...
	if (menu != NULL)
	{
		return soli_encodings_combo_box_get_selected_encoding (SOLI_ENCODINGS_COMBO_BOX (menu));
	}
...
GtkWidget *
soli_file_already_open_warning_info_bar_new (GFile *location)
{
	GtkWidget *info_bar;
	GtkWidget *hbox_content;
	GtkWidget *vbox;
...
GtkWidget *
soli_externally_modified_saving_error_info_bar_new (GFile        *location,
						     const GError *error)
{
	GtkWidget *info_bar;
	GtkWidget *hbox_content;
...
GtkWidget *
soli_no_backup_saving_error_info_bar_new (GFile        *location,
					   const GError *error)
{
	GtkWidget *info_bar;
	GtkWidget *hbox_content;
	...
	editor_settings = g_settings_new ("ca.dluco.soli.preferences.editor");
...
GtkWidget *
soli_unrecoverable_saving_error_info_bar_new (GFile        *location,
					       const GError *error)
{
	gchar *error_message = NULL;
	gchar *message_details = NULL;
	 ...
	 */
	else if (error->domain == SOLI_DOCUMENT_ERROR &&
		 error->code == SOLI_DOCUMENT_ERROR_TOO_BIG)
	{
		message_details = g_strdup (_("The disk where you are trying to save the file has "
...
GtkWidget *
soli_externally_modified_info_bar_new (GFile    *location,
...
GtkWidget *
soli_invalid_character_info_bar_new (GFile *location)
{
	GtkWidget *info_bar;
	GtkWidget *hbox_content;
	GtkWidget *vbox;
