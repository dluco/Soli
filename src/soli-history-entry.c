...
#define SOLI_HISTORY_ENTRY_HISTORY_LENGTH_DEFAULT 10

struct _SoliHistoryEntry
{
...
G_DEFINE_TYPE (SoliHistoryEntry, soli_history_entry, GTK_TYPE_COMBO_BOX_TEXT)

static void
soli_history_entry_set_property (GObject      *object,
				  guint         prop_id,
				  const GValue *value,
				  GParamSpec   *spec)
{
	SoliHistoryEntry *entry;

	g_return_if_fail (SOLI_IS_HISTORY_ENTRY (object));

	entry = SOLI_HISTORY_ENTRY (object);
	...
	switch (prop_id)
	{
		...
		case PROP_HISTORY_LENGTH:
			soli_history_entry_set_history_length (entry,
								g_value_get_uint (value));
			break;
		case PROP_ENABLE_COMPLETION:
			soli_history_entry_set_enable_completion (entry,
...
static void
soli_history_entry_get_property (GObject    *object,
				  guint       prop_id,
				  GValue     *value,
				  GParamSpec *spec)
{
	...
	switch (prop_id)
	{
		...
		case PROP_HISTORY_LENGTH:
			...
			break;
		case PROP_ENABLE_COMPLETION:
			g_value_set_boolean (value, soli_history_entry_get_enable_completion (SOLI_HISTORY_ENTRY (object)));
			break;
		default:
			G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, spec);
	}
...
static void
soli_history_entry_dispose (GObject *object)
{
	SoliHistoryEntry *entry = SOLI_HISTORY_ENTRY (object);

	soli_history_entry_set_enable_completion (entry, FALSE);

	g_clear_object (&entry->settings);

	G_OBJECT_CLASS (soli_history_entry_parent_class)->dispose (object);
}

static void
soli_history_entry_finalize (GObject *object)
{
	SoliHistoryEntry *entry = SOLI_HISTORY_ENTRY (object);

	g_free (entry->history_id);

	G_OBJECT_CLASS (soli_history_entry_parent_class)->finalize (object);
}

static void
soli_history_entry_load_history (SoliHistoryEntry *entry)
{
	gchar **items;
	gsize i;
...
static void
soli_history_entry_class_init (SoliHistoryEntryClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->set_property = soli_history_entry_set_property;
	object_class->get_property = soli_history_entry_get_property;
	object_class->dispose = soli_history_entry_dispose;
	object_class->finalize = soli_history_entry_finalize;
	...
	properties[PROP_HISTORY_LENGTH] =
		g_param_spec_uint ("history-length",
		                   ...
		                   0,
		                   G_MAXUINT,
		                   SOLI_HISTORY_ENTRY_HISTORY_LENGTH_DEFAULT,
...
static GtkListStore *
get_history_store (SoliHistoryEntry *entry)
{
	GtkTreeModel *store;
...
static gchar **
get_history_items (SoliHistoryEntry *entry)
{
	GtkListStore *store;
	GtkTreeIter iter;
	GPtrArray *array;
...
static void
soli_history_entry_save_history (SoliHistoryEntry *entry)
{
	gchar **items;

	g_return_if_fail (SOLI_IS_HISTORY_ENTRY (entry));
...
static gboolean
remove_item (SoliHistoryEntry *entry,
	     const gchar       *text)
{
	GtkListStore *store;
	GtkTreeIter iter;
...
static void
insert_history_item (SoliHistoryEntry *entry,
		     const gchar       *text,
		     gboolean           prepend)
{
	GtkListStore *store;
	...
	soli_history_entry_save_history (entry);
}

void
soli_history_entry_prepend_text (SoliHistoryEntry *entry,
				  const gchar       *text)
{
	g_return_if_fail (SOLI_IS_HISTORY_ENTRY (entry));
...
void
soli_history_entry_append_text (SoliHistoryEntry *entry,
				 const gchar       *text)
{
	g_return_if_fail (SOLI_IS_HISTORY_ENTRY (entry));
	g_return_if_fail (text != NULL);
...
void
soli_history_entry_clear (SoliHistoryEntry *entry)
{
	g_return_if_fail (SOLI_IS_HISTORY_ENTRY (entry));
...
static void
soli_history_entry_init (SoliHistoryEntry *entry)
{
	entry->history_id = NULL;
	entry->history_length = SOLI_HISTORY_ENTRY_HISTORY_LENGTH_DEFAULT;

	entry->completion = NULL;

	entry->settings = g_settings_new ("ca.dluco.soli.state.history-entry");
}

void
soli_history_entry_set_history_length (SoliHistoryEntry *entry,
					guint              history_length)
{
	g_return_if_fail (SOLI_IS_HISTORY_ENTRY (entry));
...
guint
soli_history_entry_get_history_length (SoliHistoryEntry *entry)
{
	g_return_val_if_fail (SOLI_IS_HISTORY_ENTRY (entry), 0);

	return entry->history_length;
}

void
soli_history_entry_set_enable_completion (SoliHistoryEntry *entry,
					   gboolean           enable)
{
	g_return_if_fail (SOLI_IS_HISTORY_ENTRY (entry));
	...
	if (enable)
	{
		...
		gtk_entry_set_completion (GTK_ENTRY (soli_history_entry_get_entry (entry)),
					  entry->completion);
	}
	else
	{
		...
		gtk_entry_set_completion (GTK_ENTRY (soli_history_entry_get_entry (entry)), NULL);
		g_clear_object (&entry->completion);
	}
}

gboolean
soli_history_entry_get_enable_completion (SoliHistoryEntry *entry)
{
	g_return_val_if_fail (SOLI_IS_HISTORY_ENTRY (entry), FALSE);

	return entry->completion != NULL;
}

GtkWidget *
soli_history_entry_new (const gchar *history_id,
			 gboolean     enable_completion)
{
	...
	entry = g_object_new (SOLI_TYPE_HISTORY_ENTRY,
	                      "has-entry", TRUE,
	                      "entry-text-column", 0,
	                      "id-column", 1,
	 ...
	 */
	soli_history_entry_load_history (entry);
 ...
 * simply make the SoliHistoryEntry widget implement the
 * GtkEditable interface. Unfortunately both GtkEditable and
 * GtkComboBox have a "changed" signal and I am not sure how to
 * handle the conflict.
 */
GtkWidget *
soli_history_entry_get_entry (SoliHistoryEntry *entry)
{
	g_return_val_if_fail (SOLI_IS_HISTORY_ENTRY (entry), NULL);
