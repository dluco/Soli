...
/**
 * SECTION:soli-message
 * @short_description: message bus message object
 * @include: soli/soli-message.h
 *
 * Communication on a #SoliMessageBus is done through messages. Messages are
 * sent over the bus and received by connecting callbacks on the message bus.
 * A #SoliMessage is an instantiation of a #SoliMessageType, containing
...
G_DEFINE_TYPE_WITH_PRIVATE (SoliMessage, soli_message, G_TYPE_OBJECT)

static void
soli_message_finalize (GObject *object)
{
	SoliMessage *message = SOLI_MESSAGE (object);

	g_free (message->priv->object_path);
	g_free (message->priv->method);

	G_OBJECT_CLASS (soli_message_parent_class)->finalize (object);
}

static void
soli_message_get_property (GObject    *object,
                            guint       prop_id,
                            GValue     *value,
                            GParamSpec *pspec)
{
	SoliMessage *msg = SOLI_MESSAGE (object);
...
static void
soli_message_set_property (GObject      *object,
                            guint         prop_id,
                            const GValue *value,
                            GParamSpec   *pspec)
{
...
static void
soli_message_class_init (SoliMessageClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	object_class->finalize = soli_message_finalize;

	object_class->get_property = soli_message_get_property;
	object_class->set_property = soli_message_set_property;

	/**
	 * SoliMessage:object_path:
	 *
	 * The messages object path (e.g. /soli/object/path).
	 *
	 */
...
static void
soli_message_init (SoliMessage *self)
{
	self->priv = soli_message_get_instance_private (self);
}

/**
 * soli_message_get_method:
 * @message: the #SoliMessage
 *
 ...
 */
const gchar *
soli_message_get_method (SoliMessage *message)
{
	g_return_val_if_fail (SOLI_IS_MESSAGE (message), NULL);

	return message->priv->method;
}

/**
 * soli_message_get_object_path:
 * @message: the #SoliMessage
 *
 ...
 */
const gchar *
soli_message_get_object_path (SoliMessage *message)
{
	g_return_val_if_fail (SOLI_IS_MESSAGE (message), NULL);
...
/**
 * soli_message_is_valid_object_path:
 ...
 */
gboolean
soli_message_is_valid_object_path (const gchar *object_path)
{
	if (!object_path)
	{
		return FALSE;
 ...
 */
gchar *
soli_message_type_identifier (const gchar *object_path,
                               const gchar *method)
{
 ...
 */
gboolean
soli_message_has (SoliMessage *message,
                   const gchar  *propname)
{
	GObjectClass *klass;

	g_return_val_if_fail (SOLI_IS_MESSAGE (message), FALSE);
...
gboolean
soli_message_type_has (GType         gtype,
                        const gchar  *propname)
{
	GObjectClass *klass;
	gboolean ret;

	g_return_val_if_fail (g_type_is_a (gtype, SOLI_TYPE_MESSAGE), FALSE);
	g_return_val_if_fail (propname != NULL, FALSE);
...
gboolean
soli_message_type_check (GType         gtype,
                          const gchar  *propname,
                          GType         value_type)
{
	GObjectClass *klass;
