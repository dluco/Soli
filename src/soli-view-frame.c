...
G_DEFINE_TYPE (SoliViewFrame, soli_view_frame, GTK_TYPE_OVERLAY)

static SoliDocument *
get_document (SoliViewFrame *frame)
{
	return SOLI_DOCUMENT (gtk_text_view_get_buffer (GTK_TEXT_VIEW (frame->view)));
}

static void
get_iter_at_start_mark (SoliViewFrame *frame,
			GtkTextIter    *iter)
{
...
static void
soli_view_frame_dispose (GObject *object)
{
	SoliViewFrame *frame = SOLI_VIEW_FRAME (object);
	...
	if (buffer != NULL)
	{
		GtkSourceFile *file = soli_document_get_file (SOLI_DOCUMENT (buffer));
	...
	G_OBJECT_CLASS (soli_view_frame_parent_class)->dispose (object);
}
...
static void
hide_search_widget (SoliViewFrame *frame,
                    gboolean        cancel)
{
	GtkTextBuffer *buffer;
	...
	if (cancel && frame->start_mark != NULL)
	{
		...
		soli_view_scroll_to_cursor (frame->view);
	}
...
static gboolean
search_entry_flush_timeout (SoliViewFrame *frame)
{
	frame->flush_timeout_id = 0;
...
static void
renew_flush_timeout (SoliViewFrame *frame)
{
	if (frame->flush_timeout_id != 0)
	{
		g_source_remove (frame->flush_timeout_id);
...
get_search_context (SoliViewFrame *frame)
{
	SoliDocument *doc;
...
static void
set_search_state (SoliViewFrame *frame,
		  SearchState     state)
{
	GtkStyleContext *context;

	context = gtk_widget_get_style_context (frame->search_entry);
...
static void
finish_search (SoliViewFrame    *frame,
	       gboolean           found)
{
	const gchar *entry_text = gtk_entry_get_text (frame->search_entry);
...
static void
start_search (SoliViewFrame *frame)
{
	GtkSourceSearchContext *search_context;
	GtkTextIter start_at;
...
static void
forward_search (SoliViewFrame *frame)
{
	GtkTextIter start_at;
	GtkTextBuffer *buffer;
	GtkSourceSearchContext *search_context;
...
static void
backward_search (SoliViewFrame *frame)
{
	GtkTextIter start_at;
	GtkTextBuffer *buffer;
	GtkSourceSearchContext *search_context;
...
static gboolean
remove_entry_tag_timeout_cb (SoliViewFrame *frame)
{
	frame->remove_entry_tag_timeout_id = 0;
...
static void
update_entry_tag (SoliViewFrame *frame)
{
	GtkSourceSearchContext *search_context;
	GtkTextBuffer *buffer;
...
static gboolean
update_entry_tag_idle_cb (SoliViewFrame *frame)
{
	frame->idle_update_entry_tag_id = 0;
...
static void
install_update_entry_tag_idle (SoliViewFrame *frame)
{
	if (frame->idle_update_entry_tag_id == 0)
	{
...
static void
update_search_text (SoliViewFrame *frame)
{
	const gchar *entry_text = gtk_entry_get_text (frame->search_entry);
...
static void
add_popup_menu_items (SoliViewFrame *frame,
		      GtkWidget      *menu)
{
	GtkWidget *menu_item;
	gboolean val;
...
static void
popup_menu_hide_cb (SoliViewFrame *frame)
{
	renew_flush_timeout (frame);
...
static void
setup_popup_menu (SoliViewFrame *frame,
		  GtkWidget      *menu)
{
	if (frame->flush_timeout_id != 0)
	{
                           ...
                           SoliViewFrame       *frame)
{
	...
	gtk_menu_popup (GTK_MENU (menu),
	                NULL, NULL,
	                soli_utils_menu_position_under_widget, entry,
...
static void
customize_for_search_mode (SoliViewFrame *frame)
{
	GIcon *icon;
	gint width_request;
...
static void
update_goto_line (SoliViewFrame *frame)
{
	...
	doc = get_document (frame);
	moved = soli_document_goto_line (doc, line);
	moved_offset = soli_document_goto_line_offset (doc, line, line_offset);
...
static void
init_search_entry (SoliViewFrame *frame)
{
	if (frame->search_mode == GOTO_LINE)
	{
		gint line;
	...
	else
	{
		...
		if (search_context == NULL)
		{
			...
			soli_document_set_search_context (SOLI_DOCUMENT (buffer),
							   search_context);
...
static void
start_interactive_search_real (SoliViewFrame *frame,
...
soli_view_frame_class_init (SoliViewFrameClass *klass)
{
	...
	object_class->dispose = soli_view_frame_dispose;
	object_class->finalize = soli_view_frame_finalize;

	/* Bind class to template */
	gtk_widget_class_set_template_from_resource (widget_class,
	                                             "/ca/dluco/soli/ui/soli-view-frame.ui");
	gtk_widget_class_bind_template_child (widget_class, SoliViewFrame, view);
	gtk_widget_class_bind_template_child (widget_class, SoliViewFrame, view_centering);
	gtk_widget_class_bind_template_child (widget_class, SoliViewFrame, map_frame);
	gtk_widget_class_bind_template_child (widget_class, SoliViewFrame, revealer);
	gtk_widget_class_bind_template_child (widget_class, SoliViewFrame, search_entry);
	gtk_widget_class_bind_template_child (widget_class, SoliViewFrame, go_up_button);
	gtk_widget_class_bind_template_child (widget_class, SoliViewFrame, go_down_button);
}
...
soli_view_frame_init (SoliViewFrame *frame)
{
	...
	frame->editor_settings = g_settings_new ("ca.dluco.soli.preferences.editor");
...
//				       "soli-search-entry-occurrences-tag");
...
void
soli_view_frame_popup_search (SoliViewFrame *frame)
{
	g_return_if_fail (SOLI_IS_VIEW_FRAME (frame));

	start_interactive_search_real (frame, SEARCH);
}

void
soli_view_frame_popup_goto_line (SoliViewFrame *frame)
{
	g_return_if_fail (SOLI_IS_VIEW_FRAME (frame));
...
void
soli_view_frame_clear_search (SoliViewFrame *frame)
{
	g_return_if_fail (SOLI_IS_VIEW_FRAME (frame));
