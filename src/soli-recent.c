...
void
soli_recent_add_document (SoliDocument *document)
{
	GtkSourceFile *file;
	GFile *location;
	...
	recent_data.mime_type = soli_document_get_mime_type (document);
...
void
soli_recent_remove_if_local (GFile *location)
{
...
/* The SoliRecentConfiguration struct is allocated and owned by the caller */
void
soli_recent_configuration_init_default (SoliRecentConfiguration *config)
{
	GSettings *settings;
	...
	settings = g_settings_new ("ca.dluco.soli.preferences.editor");

	g_settings_get (settings,
	                SOLI_SETTINGS_MAX_RECENTS,
	                "u",
	                &config->limit);
...
/* The SoliRecentConfiguration struct is owned and destroyed by the caller */
void
soli_recent_configuration_destroy (SoliRecentConfiguration *config)
{
...
GList *
soli_recent_get_items (SoliRecentConfiguration *config)
{
	GtkRecentFilterFlags needed;
	GList *items;
	GList *retitems = NULL;
