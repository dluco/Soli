...
#include "soli-file-chooser-dialog-gtk.h"
...
struct _SoliFileChooserDialogGtk
{
...
static void soli_file_chooser_dialog_gtk_chooser_init (gpointer g_iface, gpointer iface_data);

G_DEFINE_TYPE_EXTENDED (SoliFileChooserDialogGtk,
                        soli_file_chooser_dialog_gtk,
                        GTK_TYPE_FILE_CHOOSER_DIALOG,
                        0,
                        G_IMPLEMENT_INTERFACE (SOLI_TYPE_FILE_CHOOSER_DIALOG,
                                               soli_file_chooser_dialog_gtk_chooser_init))
...
{
	SoliFileChooserDialogGtk *dialog_gtk = SOLI_FILE_CHOOSER_DIALOG_GTK (dialog);

	g_return_if_fail (SOLI_IS_ENCODINGS_COMBO_BOX (dialog_gtk->option_menu));

	soli_encodings_combo_box_set_selected_encoding (SOLI_ENCODINGS_COMBO_BOX (dialog_gtk->option_menu),
	                                                 encoding);
}
...
chooser_get_encoding (SoliFileChooserDialog *dialog)
{
	...
	g_return_val_if_fail (SOLI_IS_ENCODINGS_COMBO_BOX (dialog_gtk->option_menu), NULL);
	...
	return soli_encodings_combo_box_get_selected_encoding (
				SOLI_ENCODINGS_COMBO_BOX (dialog_gtk->option_menu));
}
...
static GtkWindow *
chooser_get_window (SoliFileChooserDialog *dialog)
{
	return GTK_WINDOW (dialog);
}

static void
chooser_add_pattern_filter (SoliFileChooserDialog *dialog,
                            const gchar            *name,
                            const gchar            *pattern)
{
...
static void
soli_file_chooser_dialog_gtk_chooser_init (gpointer g_iface,
                                            gpointer iface_data)
{
...
static void
soli_file_chooser_dialog_gtk_dispose (GObject *object)
{
	SoliFileChooserDialogGtk *dialog_gtk = SOLI_FILE_CHOOSER_DIALOG_GTK (object);

	g_clear_object (&dialog_gtk->filter_settings);

	G_OBJECT_CLASS (soli_file_chooser_dialog_gtk_parent_class)->dispose (object);
}

static void
soli_file_chooser_dialog_gtk_class_init (SoliFileChooserDialogGtkClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->dispose = soli_file_chooser_dialog_gtk_dispose;
}

static void
create_option_menu (SoliFileChooserDialogGtk *dialog,
                    SoliFileChooserFlags      flags)
{
	...
	save_mode = (flags & SOLI_FILE_CHOOSER_SAVE) != 0;
	menu = soli_encodings_combo_box_new (save_mode);
...
static void
update_newline_visibility (SoliFileChooserDialogGtk *dialog)
{
...
static void
create_newline_combo (SoliFileChooserDialogGtk *dialog)
{
	...
	newline_combo_append (GTK_COMBO_BOX (combo),
	                      store,
	                      &iter,
	                      soli_utils_newline_type_to_string (GTK_SOURCE_NEWLINE_TYPE_LF),
	                      GTK_SOURCE_NEWLINE_TYPE_LF);

	newline_combo_append (GTK_COMBO_BOX (combo),
	                      store,
	                      &iter,
	                      soli_utils_newline_type_to_string (GTK_SOURCE_NEWLINE_TYPE_CR),
	                      GTK_SOURCE_NEWLINE_TYPE_CR);

	newline_combo_append (GTK_COMBO_BOX (combo),
	                      store,
	                      &iter,
	                      soli_utils_newline_type_to_string (GTK_SOURCE_NEWLINE_TYPE_CR_LF),
...
static void
create_extra_widget (SoliFileChooserDialogGtk *dialog,
                     SoliFileChooserFlags      flags)
{
...
static void
action_changed (SoliFileChooserDialogGtk *dialog,
		GParamSpec                *pspec,
		gpointer                   data)
{
...
static void
filter_changed (SoliFileChooserDialogGtk *dialog,
		GParamSpec	       *pspec,
		gpointer		data)
{
	GtkFileFilter *filter;
	...
	if (filter != NULL)
	{
		...
		soli_debug_message (DEBUG_COMMANDS, "Active filter: %s (%d)", name, id);

		g_settings_set_int (dialog->filter_settings,
				    SOLI_SETTINGS_ACTIVE_FILE_FILTER, id);
	}
}
...
static void
soli_file_chooser_dialog_gtk_init (SoliFileChooserDialogGtk *dialog)
{
	dialog->filter_settings = g_settings_new ("ca.dluco.soli.state.file-filter");
}

SoliFileChooserDialog *
soli_file_chooser_dialog_gtk_create (const gchar             *title,
			              GtkWindow               *parent,
			              SoliFileChooserFlags    flags,
			              const GtkSourceEncoding *encoding,
...
{
	SoliFileChooserDialogGtk *result;
	GtkFileFilter *filter;
	gint active_filter;
	GtkFileChooserAction action;
	...
	result = g_object_new (SOLI_TYPE_FILE_CHOOSER_DIALOG_GTK,
			       "title", title,
			       "local-only", FALSE,
			       "action", action,
	...
	if (encoding != NULL)
	{
		soli_encodings_combo_box_set_selected_encoding (SOLI_ENCODINGS_COMBO_BOX (result->option_menu),
		                                                 encoding);
	}

	active_filter = g_settings_get_int (result->filter_settings, SOLI_SETTINGS_ACTIVE_FILE_FILTER);
	soli_debug_message (DEBUG_COMMANDS, "Active filter: %d", active_filter);

	if ((flags & SOLI_FILE_CHOOSER_ENABLE_DEFAULT_FILTERS) != 0)
	{
		/* Filters */
		filter = gtk_file_filter_new ();
	...
	return SOLI_FILE_CHOOSER_DIALOG (result);
}
