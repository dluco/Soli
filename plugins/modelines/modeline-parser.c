...
/* Mappings: language name -> Soli language ID */
...
load_language_mappings_group (GKeyFile *key_file, const gchar *group)
{
	...
	soli_debug_message (DEBUG_PLUGINS,
	...
	while (s != NULL && *s != '\0')
	{
		...
		if ((line_number <= 3 || line_number > line_count - 3) &&
		    (strncmp (s, "ex:", 3) == 0 ||
		     ...
		     strncmp (s, "vim:", 4) == 0))
		{
			soli_debug_message (DEBUG_PLUGINS, "Vim modeline on line %d", line_number);
		...
		{
			soli_debug_message (DEBUG_PLUGINS, "Emacs modeline on line %d", line_number);
			...
			s = parse_emacs_modeline (s + 3, options);
			 ...
			 strncmp (s, "kate:", 5) == 0)
		{
			soli_debug_message (DEBUG_PLUGINS, "Kate modeline on line %d", line_number);
...
modeline_parser_apply_modeline (GtkSourceView *view)
{
	...
	if (has_option (&options, MODELINE_SET_LANGUAGE) && options.language_id)
	{
		...
		{
			soli_document_set_language (SOLI_DOCUMENT (buffer),
			                             NULL);
		}
		else
		{
	...
	else if (check_previous (view, previous, MODELINE_SET_INSERT_SPACES))
	{
		...
		insert_spaces = g_settings_get_boolean (settings, SOLI_SETTINGS_INSERT_SPACES);
	...
	else if (check_previous (view, previous, MODELINE_SET_TAB_WIDTH))
	{
		...
		g_settings_get (settings, SOLI_SETTINGS_TABS_SIZE, "u", &tab_width);
	...
	else if (check_previous (view, previous, MODELINE_SET_RIGHT_MARGIN_POSITION))
	{
		...
		g_settings_get (settings, SOLI_SETTINGS_RIGHT_MARGIN_POSITION, "u",
				&right_margin_pos);
