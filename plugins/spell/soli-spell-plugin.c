...
#ifdef G_OS_WIN32
#define SOLI_METADATA_ATTRIBUTE_SPELL_LANGUAGE "spell-language"
#define SOLI_METADATA_ATTRIBUTE_SPELL_ENABLED  "spell-enabled"
#else
#define SOLI_METADATA_ATTRIBUTE_SPELL_LANGUAGE "metadata::soli-spell-language"
#define SOLI_METADATA_ATTRIBUTE_SPELL_ENABLED  "metadata::soli-spell-enabled"
#endif
...
struct _SoliSpellPluginPrivate
{
	SoliWindow *window;
};
...
G_DEFINE_DYNAMIC_TYPE_EXTENDED (SoliSpellPlugin,
				soli_spell_plugin,
				PEAS_TYPE_EXTENSION_BASE,
				0,
				G_IMPLEMENT_INTERFACE_DYNAMIC (SOLI_TYPE_WINDOW_ACTIVATABLE,
							       soli_window_activatable_iface_init)
				G_ADD_PRIVATE_DYNAMIC (SoliSpellPlugin))

static void
soli_spell_plugin_set_property (GObject      *object,
				 guint         prop_id,
				 const GValue *value,
				 GParamSpec   *pspec)
{
	SoliSpellPlugin *plugin = SOLI_SPELL_PLUGIN (object);
...
static void
soli_spell_plugin_get_property (GObject    *object,
				 guint       prop_id,
				 GValue     *value,
				 GParamSpec *pspec)
{
...
static void
soli_spell_plugin_dispose (GObject *object)
{
	SoliSpellPlugin *plugin = SOLI_SPELL_PLUGIN (object);

	soli_debug_message (DEBUG_PLUGINS, "SoliSpellPlugin disposing");

	g_clear_object (&plugin->priv->window);

	G_OBJECT_CLASS (soli_spell_plugin_parent_class)->dispose (object);
}

static void
soli_spell_plugin_class_init (SoliSpellPluginClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->set_property = soli_spell_plugin_set_property;
	object_class->get_property = soli_spell_plugin_get_property;
	object_class->dispose = soli_spell_plugin_dispose;

	g_object_class_override_property (object_class, PROP_WINDOW, "window");
}

static void
soli_spell_plugin_class_finalize (SoliSpellPluginClass *klass)
{
}

static void
soli_spell_plugin_init (SoliSpellPlugin *plugin)
{
	soli_debug_message (DEBUG_PLUGINS, "SoliSpellPlugin initializing");

	plugin->priv = soli_spell_plugin_get_instance_private (plugin);
}

static GspellChecker *
get_spell_checker (SoliDocument *doc)
{
...
get_language_from_metadata (SoliDocument *doc)
{
	const GspellLanguage *lang = NULL;
	gchar *language_code = NULL;

	language_code = soli_document_get_metadata (doc, SOLI_METADATA_ATTRIBUTE_SPELL_LANGUAGE);
...
check_spell_cb (GSimpleAction *action,
		...
		gpointer       data)
{
	SoliSpellPlugin *plugin = SOLI_SPELL_PLUGIN (data);
	...
	view = soli_window_get_active_view (priv->window);
		 ...
		 gpointer       data)
{
	...
	window_group = soli_window_get_group (priv->window);
...
{
	SoliSpellPlugin *plugin = SOLI_SPELL_PLUGIN (data);
	SoliSpellPluginPrivate *priv = plugin->priv;
	GVariant *state;
	gboolean active;
	SoliView *view;
	...
	if (view != NULL)
	{
		...
		soli_document_set_metadata (doc,
					     SOLI_METADATA_ATTRIBUTE_SPELL_ENABLED,
				...
				gpointer       data)
{
	...
	soli_debug_message (DEBUG_PLUGINS, active ? "Inline Checker activated" : "Inline Checker deactivated");
...
static void
update_ui (SoliSpellPlugin *plugin)
{
	SoliSpellPluginPrivate *priv;
	SoliTab *tab;
	SoliView *view = NULL;
	...
	tab = soli_window_get_active_tab (priv->window);
	if (tab != NULL)
	{
...
static void
setup_inline_checker_from_metadata (SoliSpellPlugin *plugin,
				    SoliView        *view)
{
	SoliDocument *doc;
	...
	enabled_str = soli_document_get_metadata (doc, SOLI_METADATA_ATTRIBUTE_SPELL_ENABLED);
	if (enabled_str != NULL)
	{
	...
	active_view = soli_window_get_active_view (plugin->priv->window);
		    ...
		    SoliDocument *doc)
{
	...
	soli_document_set_metadata (doc,
				     SOLI_METADATA_ATTRIBUTE_SPELL_LANGUAGE, language_code,
				     NULL);
}

static void
on_document_loaded (SoliDocument    *doc,
		    SoliSpellPlugin *plugin)
{
	GspellChecker *checker;
	SoliTab *tab;
	SoliView *view;
...
static void
on_document_saved (SoliDocument *doc,
		   gpointer       user_data)
{
	SoliTab *tab;
	SoliView *view;
	...
	soli_document_set_metadata (doc,
				     ...
				     inline_checking_enabled ? SPELL_ENABLED_STR : NULL,
	                             SOLI_METADATA_ATTRIBUTE_SPELL_LANGUAGE,
	                             language_code,
	                             NULL);
}

static void
activate_spell_checking_in_view (SoliSpellPlugin *plugin,
				 SoliView        *view)
{
	SoliDocument *doc;
	 ...
	 * if a SoliTab has moved to another window.
	 */
...
static void
disconnect_view (SoliSpellPlugin *plugin,
		 SoliView        *view)
{
...
static void
deactivate_spell_checking_in_view (SoliSpellPlugin *plugin,
				   SoliView        *view)
{
...
static void
tab_added_cb (SoliWindow      *window,
	      SoliTab         *tab,
	      SoliSpellPlugin *plugin)
{
	activate_spell_checking_in_view (plugin, soli_tab_get_view (tab));
}

static void
tab_removed_cb (SoliWindow      *window,
		SoliTab         *tab,
		SoliSpellPlugin *plugin)
{
	 ...
	 * documents).
	 */
	disconnect_view (plugin, soli_tab_get_view (tab));
}

static void
soli_spell_plugin_activate (SoliWindowActivatable *activatable)
{
	SoliSpellPlugin *plugin;
	SoliSpellPluginPrivate *priv;
	GList *views;
	...
	plugin = SOLI_SPELL_PLUGIN (activatable);
	priv = plugin->priv;
	...
	views = soli_window_get_views (priv->window);
	for (l = views; l != NULL; l = l->next)
	{
		activate_spell_checking_in_view (plugin, SOLI_VIEW (l->data));
	}
...
static void
soli_spell_plugin_deactivate (SoliWindowActivatable *activatable)
{
	SoliSpellPlugin *plugin;
	SoliSpellPluginPrivate *priv;
	GList *views;
	...
	for (l = views; l != NULL; l = l->next)
	{
		deactivate_spell_checking_in_view (plugin, SOLI_VIEW (l->data));
	}
}

static void
soli_spell_plugin_update_state (SoliWindowActivatable *activatable)
{
	soli_debug (DEBUG_PLUGINS);

	update_ui (SOLI_SPELL_PLUGIN (activatable));
}

static void
soli_window_activatable_iface_init (SoliWindowActivatableInterface *iface)
{
	iface->activate = soli_spell_plugin_activate;
	iface->deactivate = soli_spell_plugin_deactivate;
	iface->update_state = soli_spell_plugin_update_state;
}

G_MODULE_EXPORT void
peas_register_types (PeasObjectModule *module)
{
	soli_spell_plugin_register_type (G_TYPE_MODULE (module));
	soli_spell_app_activatable_register (G_TYPE_MODULE (module));

	peas_object_module_register_extension_type (module,
						    SOLI_TYPE_WINDOW_ACTIVATABLE,
						    SOLI_TYPE_SPELL_PLUGIN);
}
